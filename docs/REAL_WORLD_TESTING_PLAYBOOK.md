...
For the PE-5/PE-6 grouped execution boundary, implementation milestones are developed and tested locally in order without opening, merging, or waiting for separate PRs. Full repository validation and exact-head GitHub CI run only on the final reviewed head of `PE5-IMPLEMENT-1` or `PE6-IMPLEMENT-1`; final packet-state documentation is included before that run. `PE5-CLOSE-1` and `PE6-CLOSE-1` remain separate independent audit/repair PRs and each receives its own final exact-head CI and post-merge `main` verification. A trivial documentation/status push after a successful final CI is not used; required final state is included before CI.
