...
The Adaptive Fusion Routing track approved on 2026-06-21 extends `model_selector`, `feedback`, `provider`, and the existing workflow/executor boundaries without creating a parallel routing, policy, workflow, or storage kernel. AF-0 provides pure portfolio planning, AF-1 provides a bounded in-memory endpoint registry, and AF-2 provides bounded offline replay and recommendations. AF-3 adds explicit `adaptive_provider` workflow execution for single, ordered fallback, or serial panel/judge/synthesizer plans. AF-3 requires both provider and adaptive env gates, configured API auth plus `dispatch:execute`, fixed provider/model bindings, symbolic credential env references, global and per-plan cost limits, call/token/time/concurrency ceilings, redacted/capped outputs, full provider audit events, circuit breakers, and a kill switch. It has no automatic planner-to-live routing bridge; only an explicit node plan and tick can invoke it.
