...
Product-managed Codex (API-key path) uses the Rust-owned loopback `CodexBudgetGateway` plus bubblewrap filesystem isolation (`codex_mediation_admission`): the child receives only a one-use gateway session token and an ephemeral `CODEX_HOME` with empty `auth.json`; the real upstream credential never leaves the parent gateway. The gateway is the cross-call and single-request output authority; session JSONL is corroborating evidence only. Official ChatGPT-auth Codex remains excluded from Product Golden Path admission.
