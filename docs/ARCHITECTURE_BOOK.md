...
Product-managed Codex (API-key path) uses the Rust-owned loopback `CodexBudgetGateway` plus bubblewrap filesystem+PID isolation (`codex_mediation_admission`): the child receives only a one-use gateway session token and an ephemeral `CODEX_HOME` with empty `auth.json`; the real upstream credential never leaves the parent gateway. Usage reservation/commit lives in a **parent-owned** fail-closed journal (`codex_usage_journal.v2`) outside every child sandbox mount. Provider kind/host/base URL/admitted paths are bound into `CodexBudgetAuthority`. The gateway is the cross-call and single-request output authority; session JSONL is corroborating evidence only. Admission class is currently `mediation_hardened_partial` (not full): Codex internal retries are not wire-labeled, and loopback-only network isolation remains unproved. Official ChatGPT-auth Codex remains excluded from Product Golden Path admission.
