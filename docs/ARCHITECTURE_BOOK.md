...
Production attestation uses the existing `.github/workflows/release.yml` build jobs with GitHub's official `actions/attest` v4.1.1 pinned to commit `a1948c3f048ba23858d222213b7c278aabede763`, `id-token: write`, `attestations: write`, and `contents: read`. The trust decision follows [GitHub's OIDC documentation](https://docs.github.com/en/actions/reference/security/oidc), [artifact-attestation workflow guidance](https://docs.github.com/en/actions/how-tos/secure-your-work/use-artifact-attestations/use-artifact-attestations), and the [official `gh attestation verify` policy controls](https://cli.github.com/manual/gh_attestation_verify): repository, signer workflow, tag ref, GitHub OIDC issuer, and non-self-hosted builder are constrained. No private signing key, repository signing secret, or self-hosted signing credential exists. Local dry runs use an isolated fixture identity and produce `verified_fixture`, which can never satisfy production policy.

The workflow order is build → SBOM → external attestation → provenance binding → `gh attestation verify` with explicit identity policy → verification result → existing GitHub release publication. Run-attempt, exact checked-out head, duplicate release, and clean-worktree gates prevent stale, retried, moved-head, or duplicate publication. The remote installer verifies checksum and the external attestation before extraction, validates a safe version and ordinary-file archive-member set, then routes activation through the existing `install.sh` or atomic `upgrade.sh`; the upgrader retains the prior binary/dashboard until health succeeds. Missing, malformed, or weakly bound evidence is a hard failure. `--development` is an explicit non-production source/dry-run mode only. No real tag, release, deployment, provider call, host installation, or production identity was exercised for this boundary.
...
AR-0 does not claim these are implemented and does not create infrastructure that presupposes a specific implementation.
