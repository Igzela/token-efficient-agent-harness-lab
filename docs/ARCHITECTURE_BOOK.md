...
Managed product token evidence is `product_managed_usage.v1` on the existing workflow node. Each attempt records only measured current/cumulative token counts and attempt identity; raw output is excluded. The state is committed with the node result in both SQLite and PostgreSQL and must form a contiguous attempt lineage after restart. `product_apply_binding.v2` also binds `total_calls` and `max_retries`; an excess attempt is rejected before subprocess start. Failed retryable attempts count toward the task total, missing measured usage fails closed before retry, cumulative overage becomes the existing `budget_exhausted` task state, and budget failures are nonretryable. The installed Codex CLI `0.145.0` exposes no task-scoped token-cap option and its documented JSONL usage arrives on `turn.completed`; consequently `total_tokens` is currently an authoritative post-execution measured threshold, not a pre- or during-call token cap. Time and executor-call limits remain pre-call bounds, but strict managed acceptance stays incomplete until an admitted executor can enforce a task-scoped token reservation/cap before or during the provider call. Effect/outcome-unknown errors retain their stronger outcome status. Cost remains explicitly unavailable when the CLI has no authoritative provider/model/pricing contract.
