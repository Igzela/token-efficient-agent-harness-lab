...
The Adaptive Fusion Routing track approved on 2026-06-21 must extend `model_selector`, `feedback`, `provider`, and the existing scheduler/executor boundaries. It must not create a parallel routing, policy, workflow, or storage kernel. AF-0 provides pure portfolio planning, AF-1 provides a bounded in-memory endpoint registry with deterministic secret-safe snapshots, and AF-2 provides bounded offline trace replay with task-class aggregates, Pareto frontiers, judge calibration, and shadow recommendations. AF-0 through AF-2 are shadow/offline only; these modules have no database, HTTP, credential-resolution, network, or live-routing authority. Later provider portfolio execution remains unavailable until its phase gates merge.
