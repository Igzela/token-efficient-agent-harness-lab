...
**Unchanged non-authorities.** No hidden mailbox, second scheduler/runtime kernel, automatic target-output merge/deploy/release authority, protected-branch write, or cloud multi-tenant isolation claim is introduced. Provider-backed embeddings, the managed LangGraph runtime, comparative live benchmarks, and external acceptance remain later explicitly gated work.
