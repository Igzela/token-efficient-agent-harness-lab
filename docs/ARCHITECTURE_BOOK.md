...
The workflow order is build → SBOM → external attestation → provenance binding → `gh attestation verify` with explicit identity policy → verification result → existing GitHub release publication. Run-attempt, exact checked-out head, duplicate release, and clean-worktree gates prevent stale, retried, moved-head, or duplicate publication. The remote installer verifies checksum and the external attestation before extraction, then routes activation through the existing `install.sh` or atomic `upgrade.sh`; the upgrader retains the prior binary/dashboard until health succeeds. Missing evidence is a hard failure. `--development` is an explicit non-production source/dry-run mode only. No real tag, release, deployment, provider call, host installation, or production identity was exercised for this boundary.
