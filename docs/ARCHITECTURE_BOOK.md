...
PE4-CONTRACT-1 records the durable replay safety boundary. PR #193 remains only a caller-asserted prototype: completeness booleans, reference-score flags, candidate definitions, and coverage claims are not evidence. PE4-CONTRACT-REPAIR-1 merged as PR #197 and replaces that authority with `policy_replay_contract.v2` and `trace_replay_evidence.v1` in `engine/src/feedback/replay_eligibility.rs`. The contract derives normalized observations, bounded original references, and canonical content hashes from `RunTrace`/`RunTraceRecorder`, persisted feedback/attribution sections, and actual evaluation sections. Candidate identity/version/definition, task class/domain/intent/objective, routing/policy binding, measurement schema, terminal outcome, latency, tokens, measured or posted cost, retries, quality, and judge/reference pairs are accepted only when present in those source-owner fields. Accepted and rejected observations, malformed/stale/duplicate/tampered evidence, incompatible cohorts, missing measurements, incomplete pricing, unmeasured/unpriced values, sparse and incomplete paired coverage, actual judge calibration, deterministic metric envelopes, and explicit scope-based OOD reasons remain visible and hash-bound. The compatibility bridge into `OfflineEvaluationEngine` accepts only accepted trace-derived evidence; the older caller-shaped constructor remains compatibility-only and cannot establish replay eligibility.
...
PE4-CANARY-1 adds a default-off bounded decision through the existing experiment owner. The decision carries scope, exact policy/candidate versions and hashes, minimum evidence, duration and rollout caps, explicit confirmation and permission, pause/kill outcomes, compensation, idempotency, and a hash-bound rollback target. It is a decision/evidence boundary only; direct full rollout and a second canary state machine are forbidden.

PE4-PROMOTION-1 extends the existing promotion owner with `AdaptivePromotionEvidenceChain`. Promotion validates hash-bound sufficient offline evidence, actual paired judge calibration when present, compatible shadow comparison, a started compensated canary, exact candidate/policy/source bindings, coverage, non-OOD status, metric guardrails, explicit confirmation and permission, and an active-policy rollback target. The caller-only auto-promotion path is fail-closed; successful promoted policy metadata stores the evidence-chain hash, and apply/rollback remain owned by the existing transactional store owner. Offline or shadow evidence alone cannot authorize promotion.
