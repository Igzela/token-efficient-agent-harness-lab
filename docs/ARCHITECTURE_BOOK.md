...
- Current version: v22. v18 adds immutable, hash-bound `budget_evidence_artifacts`; v19 adds `budget_pause_decisions`; v20 adds immutable `offline_replay_artifacts`; v21 adds recorder-owned trace schema/hash bindings to `dispatch_history`; v22 adds atomic Agent Runtime action receipts, explicit configured-allowlist profiles, and one-use hash-bound tool execution authorizations. Auto-pause remains default-off and requires explicit `dispatch:execute` confirmation plus validated, fresh, pricing-complete, high-confidence critical anomaly evidence scoped to the target run. Agent action application, tool authorization consumption, audit, and authoritative mutations use the existing SQLite/PostgreSQL transaction owners. Offline replay recording remains idempotent, metadata-only audited, and hash-validated. No second scheduler, provider/model authority, target-output authority, or cloud multi-tenant isolation claim is added. Normal rollback is a code revert that leaves additive tables inert so receipts and authorizations continue preventing duplicate effects. The bounded destructive `rollback_v22_to_v21` path is explicit-confirmation-only, requires stopped writers and empty v22 authority tables, writes its audit and changes the SQLite/PostgreSQL version marker in the same transaction, and refuses to discard any receipt, authorization, or configured-allowlist row. Old dispatch rows without v21 provenance remain readable but cannot establish trusted replay evidence.
...
**Unchanged non-authorities.** No hidden mailbox, second scheduler/runtime kernel, automatic target-output merge/deploy/release authority, protected-branch write, or cloud multi-tenant isolation claim is introduced. Durable cross-run semantic memory remains a later integration slice.
...
- Bounded multi-agent runtime semantics are implemented through Agent Runtime AR-0 through AR-6 and are production-routed through exact `agent_step` plans, scheduler leases, typed provider decisions, and atomic receipts. Durable cross-run semantic retrieval remains unimplemented.
