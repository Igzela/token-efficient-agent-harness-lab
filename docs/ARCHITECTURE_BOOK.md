...
- Managed CLI execution is default-off. `ACP_ENABLE_CLI_EXECUTION=1` may register Codex through its existing `workspace-write` adapter. Claude Code is currently fail-closed: identity, model, permission, and process-boundary checks exist, but provider-independent worktree-only filesystem mediation is not proved for 2.1.217, so no Claude managed admission or model request is allowed. `ACP_CLI_EXECUTION_KILL_SWITCH=1` disables both. Direct `ACP_EXECUTION_MODE=cli`, `ACP_EXECUTION_MODE=auto`, and multi/CLI dispatch are retired.
...
The managed process owner uses the versioned `managed_cli_output_limits.v1` contract. The default ceilings are 4 MiB stdout, 1 MiB stderr, and 5 MiB combined, with each configured ceiling capped at 16 MiB. Two concurrent fixed-chunk readers retain bytes only while both stream and combined ceilings remain satisfied; a breach atomically stops capture, terminates the Unix process group, reaps the parent, and waits for readers within a 1,000 ms cleanup deadline. Limit, stdout-reader, stderr-reader, wait, timeout, spawn, and process-tree-cleanup failures remain distinct typed outcomes, and no partial output is returned on a bounded-failure path. The existing `LocalProductStore` retry classifier treats every post-start managed process-boundary failure as non-retryable, preserving one-use/effect-unknown and late-write handling. Platforms without proven process-tree containment reject managed admission before process start. The Claude version probe uses this same owner with closed stdin, a cleared environment, fixed minimal `PATH`, a 2,000 ms timeout, and 1 KiB/1 KiB/2 KiB output ceilings; it revalidates the exact regular-file path and SHA-256 after probing and persists no probe output.

Normalized multi-executor usage evidence is `execution_usage_event.v1` under `engine/src/execution_usage/`: adapters map Codex JSONL, Claude Code JSONL, OpenCode read-only SQLite, and Rust provider/proxy responses into one typed event with stable dedupe identities and cross-source reconcile. Importers never become a second budget owner; ProductTask budget remains authoritative. Exact post-call usage evidence is classified separately from enforceable pre/cross-call budget authority.

Product-managed Codex (API-key path) uses the Rust-owned loopback `CodexBudgetGateway` plus bubblewrap filesystem isolation (`codex_mediation_admission`), with unprivileged user+pid namespaces when the host permits them: the child receives only a one-use gateway session token and an ephemeral `CODEX_HOME` with empty `auth.json`; the real upstream credential never leaves the parent gateway. Usage reservation/commit lives in a **parent-owned** fail-closed journal (`codex_usage_journal.v2`) outside every child sandbox mount. Provider kind/host/base URL/admitted paths are bound into `CodexBudgetAuthority`. Gateway-measured usage is mapped into the existing `execution_usage_event.v1` contract (`execution_usage/gateway_adapter.rs`); Codex/Claude/OpenCode local records remain corroborating importers only and never own ProductTask budget. Admission class is currently `mediation_hardened_partial` (not full). Residual investigation (`codex_residual_admission_finding.v1`) records `residual_admission_no_go`: Codex 0.145.0 internal retries are not wire-labeled (true retry identity unavailable); loopback-only network confinement has an executed unprivileged design proof (`unshare-net` + parent Unix gateway) but is not product-enforced (launch still shares host network with credential non-bypass only); unprivileged user/PID namespaces are host-typed (`proved` / fail-closed / unsupported / outcome-unknown). Partial-mediation live-trial authority is draft-only (`codex_partial_mediation_authority_decision.v1`; multi-field operator ack, not agent self-approve). Provider-free managed-acceptance preflight and fixture dry-run (`codex_managed_acceptance_preflight.v1`, `codex_managed_acceptance_dry_run.v1`) complete readiness so a future live run needs only parent-only credential, operator ack, and one start action. Official ChatGPT-auth Codex remains excluded from Product Golden Path admission.

Managed product token evidence is `product_managed_usage.v1` on the existing workflow node. Each attempt records only measured current/cumulative token counts and attempt identity; raw output is excluded. The state is committed with the node result in both SQLite and PostgreSQL and must form a contiguous attempt lineage after restart. `product_apply_binding.v2` also binds `total_calls` and `max_retries`; an excess attempt is rejected before subprocess start. Failed retryable attempts count toward the task total, missing measured usage fails closed before retry, cumulative overage becomes the existing `budget_exhausted` task state, and budget failures are nonretryable. The installed Codex CLI `0.145.0` remains without a proved hard pre-dispatch/during-call native token cap: `features.rollout_budget.limit_tokens` is not a hard ceiling under observed turns. Exact owner-reported usage is available from local rollout JSONL (`event_msg`/`token_count` with `total_token_usage` and `last_token_usage`, plus optional account-window `rate_limits`) and from mediated gateway counters when the product path forces API-key loopback mediation. Session logs are evidence, not a hard cross-call gate: real rollouts continue `response_item` provider rounds after `token_count` without an external interposition point. JSONL `turn.completed` stdout usage remains a complementary post-execution signal. The exact Claude admission instead pre-reserves a conservative 792,000-token ceiling for at most three 200,000-context plus 64,000-output turns, permits one scheduler call and no retry, and applies a $2.16 client-estimate cap covering base input, five-minute and one-hour cache writes, cache reads, and output under the source-dated pricing snapshot. It still records measured owner token output after the call. Claude's dollar fields are client-side estimates, not billing receipts, so canonical cost stays unavailable. Effect/outcome-unknown errors retain their stronger outcome status.
