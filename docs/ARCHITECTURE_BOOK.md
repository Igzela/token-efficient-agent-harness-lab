...
The Adaptive Fusion Routing track approved on 2026-06-21 extends `model_selector`, `feedback`, `provider`, and the existing workflow/executor boundaries without creating a parallel routing, policy, workflow, or storage kernel. AF-0 provides pure portfolio planning, AF-1 provides a bounded in-memory endpoint registry, and AF-2 provides bounded offline replay and recommendations. AF-3 adds explicit `adaptive_provider` workflow execution for single, ordered fallback, or serial panel/judge/synthesizer plans. AF-4 adds contextual policy improvement with bounded exploration, dual-gated promotion, and hash-bound rollback snapshots stored in existing `local_config` keys. AF-3/AF-4 live influence requires provider/adaptive env gates, configured API auth plus explicit workflow ticks, fixed provider/model bindings, symbolic credential env references, global and per-plan cost limits, call/token/time/concurrency ceilings, redacted/capped outputs, full provider audit events, circuit breakers, and kill switches. Promoted AF-4 policies have no live execution authority unless the workflow node also supplies explicit bounded candidate plans.
