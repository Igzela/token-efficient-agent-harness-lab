...
- Token-efficiency scorecards: `scripts/token_efficiency_scorecard.py` owns the scorecard validator; `scripts/native_scorecard_export.py` owns native bounded-summary projection and envelope export; `storage/local_product_store/native_scorecard_artifacts.rs` owns app-owned persistence; `http_server/handlers/scorecards.rs` and `operator_evidence.rs` own read-only query surfaces.
