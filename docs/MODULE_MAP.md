...
7. AC6 — schema contract (frozen in `docs/ARCHITECTURE_BOOK.md` as `PE7-AC6-CONTRACT-1`), Rust/codegen source, SDK migration, Dashboard data migration, and compatibility closeout COMPLETE (zero drift verified across 12 wire entities and 18 enums; the AC7 legacy removal candidate seed is recorded, while its exact zero-caller manifest remains pending `PE7-AC7-REMOVAL-MANIFEST-1`).
