...
- Agent Runtime (sealed at AR-6): start with `orchestration/schemas.rs` for agent/message type contracts. Storage methods: `storage/local_product_store/agent_runtime.rs`. Durable entity ownership: `AgentState` → `storage/local_product_store/schema.rs` (tables) + `agent_runtime.rs` (CRUD), `AgentMessage` mailbox → `storage/local_product_store/schema.rs` (tables) + `agent_runtime.rs` (send/read/ack/reply), `AgentStep` executor → `node_executor.rs`, agent-child-task proposals → `workflow/` and `scheduler.rs`, debate/review threads → `workflow/`, agent claim/concurrency → `scheduler.rs`, operator evidence read-model → `http_server/handlers/operator_evidence.rs`. No second scheduler, mailbox side channel, DAG kernel, or storage layer. Track is sealed; further authority expansion requires a separately approved track. See `docs/NEXT_DECISION.md` § Post-AR-6 Decision and `docs/ARCHITECTURE_BOOK.md` § Agent Runtime (AR-0) Contract.
