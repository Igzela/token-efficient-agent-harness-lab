...
| `dashboard/` | Agent-Control-Plane Phase 6 | Read-only Next.js dashboard for dispatches, routing, agents/workflows, costs, settings, and health. | App Router page, `fetchHealth`, `fetchReady`, readonly lint guard | `corepack pnpm lint`, `corepack pnpm typecheck`, `corepack pnpm build`, local HTTP smoke |
