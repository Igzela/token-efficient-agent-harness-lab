...
The T3 analysis/advice boundary creates no owner: local Sol 5.6 Max is an explicitly requested, provider-free escalation for genuinely difficult design, planning, or documentation work, not a routine reviewer or an automatic route transition. Its output is an untrusted recommendation or document draft subject to ordinary review and acceptance. It and every other model-backed path may not dispatch or execute an EFFECT, select the human disposition, submit or write any authority/outcome/T3 receipt, or replace an existing product authority, effect, approval, or outcome-evidence owner. The current T3 request binds the exact current-main authority-owner path set by digest; a separately authenticated human/operator retains the decision and the operator-only receipt transport. That transport must prove the exact request/owner binding, authenticated operator origin, and separation from model text before it writes the ledger. A redacted outcome digest is only a handoff binding: the named existing product owner retains the effect/evidence and the direct routed CLOSEOUT independently validates the owner-held outcome. The route may promote that CLOSEOUT while the EFFECT remains `IN_PROGRESS`; only the validated CLOSEOUT can advance the next successor. Missing current binding or role separation leaves the ledger unchanged and retains `T3_REQUIRED` or `DECISION_REQUIRED`; a possibly executed but unproved effect is `OUTCOME_UNKNOWN`.
