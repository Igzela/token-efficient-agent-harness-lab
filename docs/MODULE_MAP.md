...
| `engine/src/dispatch_decision.rs` | Language Migration Phase 1 | Rust dispatch decision schema structs and golden bundle parity builder. | `DispatchDecision`, `BudgetReservation`, `ExecutionGate`, `build_dispatch_bundle` | `engine/tests/dispatch_parity.rs` |
