...
| RWE production authorization/spend (Board B) | `engine/src/storage/local_product_store/rwe_authority.rs` (`issue_rwe_run_authorization_v2`, admit path) plus `engine/src/rwe/runner.rs` | Store-owned `rwe_run_authorization.v2` issue/admit and one-use spend envelope under existing `rwe_run_authorizations` row; bindings derived from freeze owners and principal, never caller checkout text; no second spend/budget owner; no provider/target effect in Board B |
