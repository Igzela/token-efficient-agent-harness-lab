...
| RWE run authorization and spend | `engine/src/storage/local_product_store/rwe_authority.rs` plus `engine/src/rwe/runner.rs` (accepted owners from PR #300; Board A v2 contract from PR #361) | Store-owned one-use RWE spend via `rwe_run_authorizations`; v1 fixture envelope and production v2 bindings remain under this single owner. Unmerged Board B production issue/admit wiring is not an accepted owner until merged. |
