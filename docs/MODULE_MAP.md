...
| `engine/src/http_server/handlers/operator_evidence.rs` | active runtime/API | AR-6 read-only operator evidence read-model: aggregates agent state, mailbox/proposal counts, blocked signals, and sanitized audit events per run | `cargo test -p engine --test test_http_server` |
...
- Token-efficiency scorecards: `scripts/token_efficiency_scorecard.py` owns the scorecard validator; `scripts/native_scorecard_export.py` owns manual native bounded-summary projection and envelope export; `scripts/langgraph_trace_import.py` owns importer-only LangGraph bounded summary normalization plus read-only scorecard comparison for stateless/stateful baselines; `storage/local_product_store/native_scorecard_artifacts.rs` owns automatic terminal-run projection, app-owned persistence, idempotent artifact recording, raw-trace rejection, and evidence-only native metric projection from workflow node results/attempts/durations; `workflow_runs.rs` owns terminal transition hooks; `http_server/handlers/scorecards.rs` and `operator_evidence.rs` own read-only query surfaces.
- Agent Runtime: start with `orchestration/schemas.rs` for agent/message type contracts. Storage methods: `storage/local_product_store/agent_runtime.rs`. Durable entity ownership: `AgentState` → `storage/local_product_store/schema.rs` (tables) + `agent_runtime.rs` (CRUD), `AgentMessage` mailbox → `storage/local_product_store/schema.rs` (tables) + `agent_runtime.rs` (send/read/ack/reply), `AgentStep` executor → `node_executor.rs`, agent-child-task proposals → `workflow/` and `scheduler.rs`, debate/review threads → `workflow/`, agent claim/concurrency → `scheduler.rs`. No second scheduler, mailbox side channel, DAG kernel, or storage layer. See `docs/ARCHITECTURE_BOOK.md` § Agent Runtime (AR-0) Contract for the full ownership table.
