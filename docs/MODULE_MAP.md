...
| Durable memory and retrieval | `durable_memory.rs`; scheduler context injection; memory HTTP handlers; SDKs; v23 SQLite/PostgreSQL tables; backup/integrity | connected end to end with exact scope/version/source hashes, bounded vector Top-K, explicit lexical degradation, restart/concurrency safety, and metadata-only retrieval evidence |
