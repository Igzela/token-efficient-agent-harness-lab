...
| Candidate workspace | supervised-patch/app-owned workspace and target-output worktree owners | isolated candidate state; registered target tree and active `main` remain protected |
...
| Candidate lineage/archive | `LocalProductStore` + `engine/src/harness_evolution.rs` + `engine/src/harness_evolution_eval.rs` + v27/v28 tables (`harness_evolution_*`), migrations, integrity, backup/restore, audit | app-owned versioned evidence with exact parent/base/content/evaluation/Pareto identities; B1+B2 foundation; no PR_READY until B3 |
