...
| Managed CLI/process | `cli/mod.rs`, `cli/config.rs`, `cli/cli_node_executor.rs`, `cli/codex_budget_authority.rs`, `cli/codex_mediation_admission.rs`, `cli/codex_residual_admission.rs`, `cli/codex_partial_mediation_authority_decision.rs`, `cli/codex_managed_acceptance_preflight.rs`, `cli/codex_usage_journal.rs`, `cli/codex_session_usage.rs` | bounded process owner, probe, parser, admission, Codex loopback budget gateway, parent journal, residual findings, partial-mediation authority draft, provider-free managed-acceptance preflight/manifest, session usage evidence |
