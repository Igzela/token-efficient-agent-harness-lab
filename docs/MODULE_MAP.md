...
| `engine/src/http_server.rs` | Language Migration Rust Engine/API Parity + Permission Governance + Cost Governance + Dashboard Controls | Rust HTTP server context plus local axum router for health, readiness, OpenAPI JSON, deterministic dispatch, team/key CRUD, cost summary, per-dispatch cost details, dispatch detail, backup management, and optional static dashboard serving. Includes auth, scope checks, rate limit checks, and CORS headers. | `ServerContext`, `ServerConfig`, `AxumApiState`, `build_axum_router`, `build_axum_router_with_dashboard`, `openapi_document` | `engine/src/http_server.rs` (inline tests), `engine/tests/test_http_server.rs` |
...
| `engine/src/storage/local_product_store.rs` | Agent-Control-Plane Local Small-Team Productization + Permission Governance + Cost Governance + Dashboard Controls | SQLite-backed app-owned local state for dispatch history, config, team/API-key metadata, audit log, cost summary (v2: reserved, estimated, tokens, utilization, daily trend), per-dispatch cost details, dashboard state, and export snapshot. | `LocalProductStore`, `local_boundaries` | `engine/tests/test_http_server.rs`, `engine/tests/test_local_product_store.rs` |
