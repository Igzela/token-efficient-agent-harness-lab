...
| Wire contracts and codegen | `wire_contract/`, `codegen/`, `engine/src/wire_types.rs` | Canonical JSON schema definitions and deterministic cross-language codegen; drift checked by `scripts/check_wire_codegen_drift.sh`; AC6 type governance contract |
...
7. AC6 — schema contract (frozen in `docs/ARCHITECTURE_BOOK.md` as `PE7-AC6-CONTRACT-1`), Rust/codegen source, SDK migration, Dashboard data migration, then compatibility closeout.
