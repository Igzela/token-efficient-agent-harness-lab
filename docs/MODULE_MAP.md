...
| RWE first live baseline composition | `engine/src/rwe/live_baseline_coordinator.rs`, thin CLI `rwe-live-baseline`; reuses Product Golden Path + `LocalProductStore` owners for exact frozen RWE bindings | Draft PR #363 only; orchestrates frozen 4-cell schedule over Board B admit, store cell fence, ProductTask/managed executor/spend journal/verifier/artifact/Draft PR/terminal/cleanup; no second scheduler/store/budget/runtime |
