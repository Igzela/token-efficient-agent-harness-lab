...
| `engine/src/http_server.rs` | Language Migration Complete | Rust HTTP server context with route registration, path matching with params, scope enforcement, and query string stripping. | `ServerContext`, `ServerConfig`, `RouteHandler` | `engine/src/http_server.rs` (inline tests) |
| `engine/src/doc_generator.rs` | Language Migration Complete | Rust documentation generator with module/schema registry, Rust source parser, and markdown generation. | `DocGenerator`, `ModuleDoc`, `parse_module_from_source` | `engine/tests/test_doc_generator.rs` |
