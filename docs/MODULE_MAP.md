...
| Managed provider-call protocol adapters | Existing `engine/src/provider/`, `engine/src/execution_usage/`, ProductTask managed-acceptance/store owners | Packet 2 extends the existing OpenAI-compatible and Anthropic adapters beneath one ProductTask-bound provider-call authority; credential resolution stays parent-only and adapters never own budget, lease, approval, output, or audit state. |
...
The next ordered proposed implementation surfaces are Packet 1 managed-coding runtime profiles plus Git/local-folder sources, Packet 2 managed DeepSeek protocol adapters, and Packet 3's separately authorized live seal. They remain proposed implementation until each final exact head is accepted and merged; PR #225 remains independent and last.
