...
| Shadow Steward projection | `scripts/agent-control/shadow_steward.py` and `tests/test_agent_shadow_steward.py` | Pure provider-free intake, digest-bound proposal evaluation, deterministic Stage/WorkCard planning/replanning, stop taxonomy, fixture replay comparison, and compact redacted status. It returns recommendations only; `mission_contract.py` owns contract validation and `local_loop.py` remains the sole lifecycle writer. No raw request, comment, private path, credential, persistence, GitHub/Provider transport, worker, service, scheduler, evaluator, budget, output, audit, rollback, or mutation authority. |
