...
| Harness Evolution evaluator and holdout | `engine/src/harness_evolution_eval.rs` plus `engine/src/storage/local_product_store/harness_evolution.rs` | Sole evaluator-owned task-family, label/rubric, sealed-vault, one-use selection, hard-gate, metric, evaluation-bundle, prediction-outcome, and Pareto-evidence boundary; fixture helpers are not managed acceptance and no second evaluator/store is allowed |
