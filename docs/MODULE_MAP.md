...
| `dashboard/` | Agent-Control-Plane Phase 6 | Read-only Next.js dashboard for dispatches, routing, agents/workflows, costs, settings, and health. | App Router page, `fetchHealth`, `fetchReady`, readonly lint guard | `pnpm lint`, `pnpm typecheck`, `pnpm build`, local HTTP smoke |
| `deploy/`, `docker-compose.yml` | Agent-Control-Plane Phase 7 | Local Docker build/run definitions for Rust API and read-only dashboard. | `deploy/Dockerfile.engine`, `deploy/Dockerfile.dashboard`, compose `engine`/`dashboard` services | `docker compose build`, `docker compose up --build -d`, `/api/v1/health`, `/api/v1/dispatch`, dashboard HTTP smoke |
| `docs/AGENT_CONTROL_PLANE_MIGRATION_CLOSEOUT.md` | Agent-Control-Plane Phase 8 | Migration closeout record for phases 0-7, verification evidence, safety-boundary evidence, and Python reference retention. | Closeout status and remaining decision | `python3 scripts/check_agent_handoff.py` |
