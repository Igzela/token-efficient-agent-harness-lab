...
| RWE first-live-baseline coordinator | `engine/src/rwe/live_baseline_coordinator.rs`, thin CLI `rwe-live-baseline` (`engine/src/bin/rwe_live_baseline.rs`) | Provider-free orchestration of the frozen 4-cell schedule over Board B admit + existing ProductTask/managed_deepseek/workspace/verification/approval/output/terminal owners via injectable `CellDriver`; no second scheduler/store/budget/provider runner |
