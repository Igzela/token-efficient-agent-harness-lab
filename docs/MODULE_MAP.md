...
- Token-efficiency scorecards: `scripts/token_efficiency_scorecard.py` owns the scorecard validator; `scripts/native_scorecard_export.py` owns manual native bounded-summary projection and envelope export; `storage/local_product_store/native_scorecard_artifacts.rs` owns automatic terminal-run projection, app-owned persistence, idempotent artifact recording, raw-trace rejection, and evidence-only native metric projection from workflow node results/attempts/durations; `workflow_runs.rs` owns terminal transition hooks; `http_server/handlers/scorecards.rs` and `operator_evidence.rs` own read-only query surfaces.
