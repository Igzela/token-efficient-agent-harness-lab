...
| Candidate lineage/archive | `LocalProductStore` + `engine/src/harness_evolution.rs` + v27 tables (`harness_evolution_*`), migrations, integrity, backup/restore, audit | app-owned versioned evidence with exact parent/base/content/evaluation identities; no second event store or vector service; B1 foundation only until B2/B3 merge |
