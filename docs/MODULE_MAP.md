...
| Managed CLI/process | `cli/mod.rs`, `cli/config.rs`, `cli/cli_node_executor.rs`, `cli/codex_budget_authority.rs`, `cli/codex_mediation_admission.rs`, `cli/codex_residual_admission.rs`, `cli/codex_partial_mediation_authority_decision.rs`, `cli/codex_managed_acceptance (decision/risk/spend/attempt v33)_preflight.rs`, `cli/codex_managed_acceptance (decision/risk/spend/attempt v33)_dry_run.rs`, `cli/codex_usage_journal.rs`, `cli/codex_session_usage.rs` | Codex loopback gateway, parent journal, residual findings, partial-mediation authority draft, managed-acceptance preflight/manifest, provider-free fixture dry-run + board readiness dossier, session usage evidence |
...
| Persistence/evidence | `storage/local_product_store/` (+ `managed_acceptance (decision/risk/spend/attempt v33).rs` v32 decisions/authorizations/attempts) | sole SQLite/PostgreSQL store, transaction, audit, evidence, managed-acceptance attempt admission, migration owner |
