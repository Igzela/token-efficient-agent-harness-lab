...
| `engine/src/provider.rs` | Language Migration Phase 2 | Rust provider boundary trait with disabled-by-default local stub; no real transport implementation. | `Provider`, `DisabledProvider`, `ProviderRequest`, `ProviderResponse`, `ProviderError` | `engine/tests/test_provider.rs` |
...
| `engine/src/http_server.rs` | Language Migration Rust Engine/API Parity | Rust HTTP server context plus local axum router for health, readiness, OpenAPI JSON, deterministic dispatch, and optional static dashboard serving. Includes auth, scope checks, rate limit checks, and CORS headers. | `ServerContext`, `ServerConfig`, `AxumApiState`, `build_axum_router`, `build_axum_router_with_dashboard`, `openapi_document` | `engine/src/http_server.rs` (inline tests), `engine/tests/test_http_server.rs` |
...
| `engine/src/wire_types.rs` | Agent-Control-Plane Phase 5 | Generated Rust wire-contract type surface for API/SDK boundary values. | `DispatchRequest`, `RequestSource`, `DispatchBundleValue`, `ApiStatus` | `python3 codegen/generate_wire_types.py`, `cargo test -p engine` |
| `sdk/typescript/` | Agent-Control-Plane Phase 5 | TypeScript REST SDK package for health, readiness, OpenAPI, and deterministic dispatch endpoints. | `AgentControlPlaneClient`, generated wire types | `pnpm build`, `npm pack --dry-run` |
...
| `dashboard/` | Agent-Control-Plane Phase 6 | Read-only Next.js dashboard for dispatches, routing, agents/workflows, costs, settings, and health, with standalone and static-export build modes. | App Router page, `fetchHealth`, `fetchReady`, readonly lint guard, `build:static` | `pnpm lint`, `pnpm typecheck`, `pnpm build`, `pnpm build:static`, native runtime smoke |
| `scripts/smoke_native_runtime.py` | Agent-Control-Plane Native Local Runtime | Stdlib smoke test that starts the Rust engine with `ACP_DASHBOARD_DIR`, then verifies health, readiness, dispatch, and dashboard root without Docker. | `main` | `python3 scripts/smoke_native_runtime.py` |
| `deploy/`, `docker-compose.yml` | Agent-Control-Plane Phase 7 | Optional local Docker build/run definitions for Rust API and read-only dashboard. | `deploy/Dockerfile.engine`, `deploy/Dockerfile.dashboard`, compose `engine`/`dashboard` services | `docker compose build`, `docker compose up --build -d`, `/api/v1/health`, `/api/v1/dispatch`, dashboard HTTP smoke |
| `docs/AGENT_CONTROL_PLANE_MIGRATION_CLOSEOUT.md` | Agent-Control-Plane Phase 8 | Migration closeout record for phases 0-7 plus native runtime hardening, verification evidence, safety-boundary evidence, and Python reference retention. | Closeout status and remaining decision | `python3 scripts/check_agent_handoff.py` |
