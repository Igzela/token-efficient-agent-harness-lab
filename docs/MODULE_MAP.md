...
| RWE first-live-baseline coordinator | `engine/src/rwe/live_baseline_coordinator.rs`, thin CLI `rwe-live-baseline` (`engine/src/bin/rwe_live_baseline.rs`); store fence on `LocalProductStore` RWE attempt owner | Provider-free orchestration of the frozen 4-cell schedule over Board B admit + store-backed cell dispatch fence/budget reservation; injectable drivers for tests only; multi-path live Product Golden Path cell composition remains DECISION_REQUIRED (no second scheduler/store/budget/provider runner) |
