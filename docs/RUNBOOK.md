...
Do not paste a credential into this command or shell history; obtain it through the trusted-local process environment. `ACP_DURABLE_MEMORY_EMBEDDING_KILL_SWITCH=1` blocks new calls, provider mode is prohibited when `CI` is set, and `fixture` remains test-only. Provider responses persist only the numeric vector, model/pricing/usage/vector hashes, scope/source/version bindings, and the hash-bound restart receipt; raw memory text is excluded from provider audit and scorecards. A `completed` receipt is reused for the exact mutation after restart without another POST. `request_sent`, `outcome_unknown`, and `failed` receipts are fail-closed and require explicit diagnosis; deleting or rewriting them to force a retry is unsupported. Lexical retrieval occurs only when the request explicitly sets `allow_lexical_fallback=true`; its result remains labeled `lexical_fallback`.
