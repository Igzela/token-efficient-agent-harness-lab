...
Endpoint JSON stores only credential environment names. Remote HTTP is rejected; HTTPS is required except for loopback test/local adapters. A legacy protected runtime with provider and adaptive execution gates plus `ACP_REQUIRE_AUTH=1` may start without `ACP_PROVIDER_TYPE` or `ACP_ADAPTIVE_PROVIDER_ENDPOINTS_JSON`; it remains fail-closed with `adaptive_provider_not_available` until endpoint config succeeds. Operators can inspect and persist endpoint metadata from the dashboard Settings provider endpoint panel or `PUT /api/v1/provider/endpoints` with `confirm_provider_endpoint_config=true`; raw API keys are rejected, and real provider entries require their referenced environment variables to exist before the config is saved. Validated local endpoint config applies to `POST /api/v1/adaptive-fusion/completions` without restart and restores the startup-bound adaptive executor and registry on the next process start. Explicit `ACP_ADAPTIVE_PROVIDER_ENDPOINTS_JSON` is authoritative while present; the API reports `environment_override_active`, and saved local config remains inactive until that env override is removed. Explicit workflow execution uses the startup-bound adaptive executor and accepts bounded `single`, `ordered_fallback`, or `fusion` plans. Fusion panels may run with bounded concurrency up to 3; judge and synthesizer remain serial. Tick the run with `executor=adaptive_provider`, `max_retries=0`, and a key with `dispatch:execute`.
