...
9. **Verification.** git diff --check; cargo fmt --all -- --check; cargo clippy -p engine --all-targets --all-features -- -D warnings; cargo test -p engine; uv run --no-project python -m unittest tests.test_verify_rwe_snapshot; uv run --python /usr/bin/python3.14 --no-project python scripts/verify_rwe_snapshot.py --manifest engine/rwe/corpora/rwe-minimum-first-corpus/v2/snapshot/pre_ac_harness_snapshot.v2.json --source-root /tmp/pe7-rwe-pre-ac-source --harness-root /tmp/pe7-rwe-post-ac-harness --post-ac-main-sha 42fcfa5ad7e349d27d3caa815163340f9c0d5c0b --post-ac-tree-sha c81a2e4e635da05a8a1c15630371e98943c70c86 --post-ac-cargo-lock-sha256 cf68982734f8a72148950f119408b676dd5b42ce65d7af69c02eca017a551653 --post-ac-rust-toolchain-sha256 e59c5da37d1f9f4e0f815bc188cb6056fc7410c9cdaa9673c2d44da557c75d12 --execute-traces; uv run --no-project python -m unittest discover -s tests -p test_agent_*.py
The exact verifier command above, including every `--post-ac-*` binding and `--execute-traces`, is the only current acceptance entrypoint. The frozen snapshot's `rebuild.commands` remains an immutable historical recipe: its final verifier command predates the isolated sibling-root and accepted-main bindings, so it is never executed or treated as current evidence, and the frozen snapshot input is never edited.
