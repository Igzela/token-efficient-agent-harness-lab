...
**Prerequisite:** PE7-AC2-BOUNDARY-CORE-1 — COMPLETE merge-backed packet; the documented review repair is carried by this packet; see its disposition in `docs/CURRENT_STATUS.md`.
...
8. **Failure and recovery.** `ProcessEffectState::Unknown` or `ProcessOutcomeState::Unknown` remains non-success and non-retryable; a contradictory mapping can never authorize success; preserve existing outcome-unknown, lease, rollback, and reconciliation owners; keep the invariant repair separable from caller adapters so recovery never restores the known defect.
...
10. **Compatibility and rollback.** No serialized field or public API changes are permitted; keep invariant/pre-spawn repair in a separate commit, and roll back caller-adapter commit(s) only while retaining that repair and the accepted AC2 contract/boundary core; if separation is impossible, stop rather than restore the known defect.
