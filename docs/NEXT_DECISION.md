...
The frozen snapshot's `rebuild.commands` remains an immutable historical recipe. Its final verifier command predates the isolated sibling-root and accepted-main bindings, so the packet's explicit command above is the only current acceptance entrypoint; the frozen snapshot input is never edited or executed with relaxed isolation.
