...
```text
[window: Route automation — continuous route driver — READY_FOR_EXECUTION, provider-free control-plane implementation only]
→ automatic successor compilation + closeout/promotion PR lifecycle
→ adversarial soak of the route driver on refreshed accepted main
→ blocked PREFLIGHT repair/reconciliation (B1/B2/provenance) then preflight smoke
...
1. `PE7-ROUTE-AUTOMATION-1` — `READY_FOR_EXECUTION`. Sole current packet: automatic successor-contract compilation from `docs/FUTURE_ROUTE.md` at promotion time (refreshed twelve-field contract + weak-agent-dispatch capsule), the canonical closeout/promotion PR lifecycle, and a continuous route driver repeating across packet boundaries without manual successor authoring. Creates no Provider, target, or T3 effect.
...
No packet other than the current one is execution-ready. `PE7-ROUTE-AUTOMATION-1` is execution-ready below; `PE7-PLAN-LANE-ACTIVATION-1`, `PE7-LIFECYCLE-CONTROLLER-1`, and `PE7-SUCCESSOR-PROMOTION-ESCALATION-1` are complete; `PE7-RWE-V2-VIABILITY-PREFLIGHT-1` is blocked pending the control-plane transition's later B1/B2/provenance reconciliation; every packet in `docs/FUTURE_ROUTE.md` is routing-only until its exact predecessor is accepted and its complete contract is moved here and refreshed against then-current `main`. An implementation agent must stop `DECISION_REQUIRED` rather than fill a missing architecture, authority, statistical, evaluator, retention, spend, recovery, or adoption decision.
...
## Completed Successor Promotion and Escalation (PE7-SUCCESSOR-PROMOTION-ESCALATION-1)

**Historical state:** `COMPLETE`

**Historical evidence:** PR #387 exact head `5fe3d55af19aa7a081115637f8f8a7aa63b581af`, merge `597f90282fb6ca72472b890b825684bf54486709`, exact-head `PASS`, canonical workflow `31403849100`; accepted receipt in `docs/CURRENT_STATUS.md`. After an accepted closeout, the lane verifies live routing, records exactly-one bounded successor-promotion receipt (packet id, accepted-main SHA, capsule digest), and escalates blockers with a bounded pause receipt — controller-owned, idempotent, no automatic EFFECT/T3 authority, no self-promotion. Full contract and retired capsule preserved in merged PR #387 and git history; must not be executed again.

## Packet PE7-ROUTE-AUTOMATION-1

**State:** `READY_FOR_EXECUTION`

**Prerequisite:** `PE7-SUCCESSOR-PROMOTION-ESCALATION-1` COMPLETE on accepted main `597f90282fb6ca72472b890b825684bf54486709` (PR #387 exact head `5fe3d55af19aa7a081115637f8f8a7aa63b581af`, workflow `31403849100`, merge `597f90282fb6ca72472b890b825684bf54486709`); ledger Issue #383 provisioned and lane active.

**Class:** `IMPLEMENT`

**Outcome:** Close the route-autopilot gap: (a) automatic successor-contract compilation from `docs/FUTURE_ROUTE.md` at promotion time — exactly one eligible successor from the checked inventory manifest, its twelve-field contract and capsule refreshed against accepted `main`; (b) the canonical closeout/promotion PR lifecycle as one Draft→checks→Ready→canonical CI→eligible merge PR; (c) a continuous route driver repeating across packet boundaries without manual successor authoring. `promote_plan` compiles the successor when absent or still current instead of only validating an already-present successor; bounded pause escalation remains only for genuine `DECISION_REQUIRED` or blocked routing.

**Allowed delta:** `scripts/agent-control/dispatcher.py`, `local_run_once.py`, `local_loop.py`, `local_supervisor.py`, `plan_lane.py`, `state_manager.py`, `control_state.py`, `loopctl.py`, new `scripts/agent-control/route_driver.py`, `scripts/session_context.py`, `scripts/check_agent_handoff.py`, their focused control-plane tests (including new `tests/test_agent_route_driver.py`), and the smallest canonical-document capsule/status sync required.

**Exit:** The accepted route driver compiles a successor's refreshed twelve-field contract + capsule from `docs/FUTURE_ROUTE.md`, drives the canonical closeout/promotion PR to an eligible merge, and repeats across at least one full packet boundary end-to-end without manual authoring; exactly one successor is promoted at a time; no successor executes; no child gains merge/GitHub/Provider/T3 authority; EFFECT execution, automatic T3, and PREFLIGHT stay inactive.

**Stop:** Any second ledger/controller/store/state/routing owner, promotion of zero or multiple successors, inventory manifest mismatch, stale or unprovable routing, self-promotion, caller- or model-asserted promotion, missing or conflicting closeout receipt, CI failed/queued/missing, review absent/stale, ineligible merge, unknown external outcome, child authority, Provider/target effect, automatic successor/EFFECT/T3 path, or requirement to choose a new schema, evaluator, retention, spend, statistical, or T3 value.

### Twelve-field contract

1. **Outcome and non-goals.** Route-level automation: promotion-time successor-contract compilation from the checked `docs/FUTURE_ROUTE.md` inventory, canonical closeout/promotion PR lifecycle, continuous route driver across packet boundaries. Non-goals: no successor execution, EFFECT, automatic T3 handling, PREFLIGHT activation, Provider/target/credential access, second controller/ledger/store/scheduler/routing owner, auto-merge without eligibility, schema/evaluator/budget/authority change, or self-promotion.
2. **Prerequisites and evidence.** Accepted main `597f90282fb6ca72472b890b825684bf54486709`; predecessor receipt is PR #387 exact head `5fe3d55af19aa7a081115637f8f8a7aa63b581af`, workflow `31403849100`, merge `597f90282fb6ca72472b890b825684bf54486709`; ledger Issue #383 provisioned and lane active; `docs/FUTURE_ROUTE.md` checked inventory manifest is the sole successor source.
3. **Owners and paths.** Sole plan lifecycle anchor: Plan Execution Ledger via `control_state.py`; controller transitions: `dispatcher.py`, `state_manager.py`; outer loop: `local_loop.py`, `local_supervisor.py`, `loopctl.py`; plan compiler/executor: `plan_lane.py`, `local_run_once.py`; promotion compiler, closeout/promotion PR lifecycle, and continuous driver: new `scripts/agent-control/route_driver.py`; routing authority: `docs/NEXT_DECISION.md` and `docs/FUTURE_ROUTE.md`; escalation owner: planning process via canonical documents and the ledger pause receipt.
4. **Frozen subject identity and invariants.** Every compiled successor binds `packet_id`, the checked inventory manifest SHA, accepted-main SHA, the closed-out predecessor's closeout identity, and the compiled capsule digest. Exactly one successor is compiled and promoted at a time; the compiled successor is never executed; the manifest is never silently edited (manifest changes are planning diffs); the driver is idempotent across restarts; no model self-report advances routing; no child receives merge credentials, GitHub authority, Provider credentials, or T3 authority; a closeout/promotion PR never auto-merges.
5. **Only semantic delta.** Add route-level automation: promotion-time contract compilation, canonical closeout/promotion PR lifecycle, continuous route driver; no new semantic plan contract, no second representation, no change to packet class contracts.
6. **Forbidden changes.** No second Issue, ledger, store, scheduler, controller, queue, lease, routing owner, evaluator, schema migration, authority boundary, Provider call, credential read, target write, automatic merge, release, deployment, successor execution, EFFECT, or automatic T3 handling; no silent `docs/FUTURE_ROUTE.md` manifest or packet-sketch edit (planning-diff only); no bypass of Draft/Ready discipline, exact-head review, canonical CI, or merge eligibility; no activation of PREFLIGHT or any EFFECT packet.
7. **Ordered implementation slices.** (a) promotion compiler: parse the checked `docs/FUTURE_ROUTE.md` inventory manifest and the eligible successor packet sketch; select exactly one successor with all prerequisites COMPLETE and a manifest-resolved class/verification profile; compile the refreshed twelve-field contract + capsule against accepted `main`; fail closed on zero/multiple/ambiguous candidates, manifest mismatch, or stale routing. (b) `promote_plan` integration: `successor_absent`/`successor_still_current` compile and record promotion instead of only escalating; bounded pause escalation remains only for genuine `DECISION_REQUIRED`/blocked routing. (c) closeout/promotion PR lifecycle: branch → docs-only closeout diff → Draft PR → checks → review → Ready → canonical CI → eligible merge → refresh `main`. (d) continuous route driver: repeat across packet boundaries with idempotent state and bounded pauses; stop before EFFECT/PREFLIGHT/T3. (e) focused/full verification.
8. **Failure, recovery, and stop taxonomy.** Fail closed on: manifest mismatch or unrefreshed manifest, zero or multiple eligible successors, prerequisite not COMPLETE, unprovable or stale routing, missing or conflicting closeout receipt, PR lifecycle conflict, CI failed/queued/missing, review absent/stale, ineligible merge, unknown external outcome, or unavailable owner. Restart reconstructs driver and promotion state idempotently from ledger plus authoritative routing; never re-promotes, re-opens a duplicate PR, or re-merges after a provable receipt; a merged closeout/promotion PR is never reverted.
9. **Verification.** Focused compiler, lifecycle, driver tests (`tests/test_agent_route_driver.py`, updated `tests/test_agent_plan_promotion.py`, `tests/test_agent_plan_lifecycle.py`); session-context/handoff tests; Python/control-plane suite; security baseline; handoff checker; `git diff --check`; exact-head canonical CI; complete-diff independent `PASS`.
10. **Compatibility, rollback, and retention.** Existing Issue lane, activation, lifecycle, promotion behavior unchanged. Revert only this packet's code/docs; retain ledger closeout and promotion receipts; do not delete failed, rejected, or outcome-unknown transitions; no schema migration or new persistence owner.
11. **Exit artifact.** Accepted route automation (compiler + closeout/promotion PR lifecycle + continuous driver) with one full end-to-end packet-boundary crossing (closeout docs PR → merge → next packet current) whose successor was compiled, not manually authored; focused/full verification; successor execution/EFFECT/T3 paths inactive.
12. **Next action.** After acceptance, the route driver promotes and runs exactly one adversarial-soak packet against refreshed accepted main; after the soak, the blocked PREFLIGHT repair/reconciliation (authoritative B1, store-owned B2, provenance disposition) is re-expanded and its provider-free S1–S5 smoke run. Do not activate successor execution, EFFECT, automatic T3 handling, or PREFLIGHT authority from this packet.

### 11. Weak-Agent Dispatch Capsule

The active capsule below binds this route-automation packet. Its effect limit
is zero; it grants no Provider, target, merge, GitHub, credential, authority,
or T3 access. The retired successor-promotion/escalation capsule is preserved
in merged PR #387 and git history; not an execution surface.

<!-- weak-agent-dispatch:v1
{"allowed_outputs":["Controller-owned promotion and escalation receipt bindings with readback evidence only","Automatic successor-contract compilation from docs/FUTURE_ROUTE.md refreshed against accepted main","Canonical closeout/promotion PR lifecycle evidence","Focused provider-free promotion, escalation, recovery, and subject-identity verification evidence"],"allowed_paths":["docs/CURRENT_STATUS.md","docs/FUTURE_ROUTE.md","docs/NEXT_DECISION.md","scripts/agent-control/control_state.py","scripts/agent-control/dispatcher.py","scripts/agent-control/local_loop.py","scripts/agent-control/local_run_once.py","scripts/agent-control/local_supervisor.py","scripts/agent-control/loopctl.py","scripts/agent-control/plan_lane.py","scripts/agent-control/route_driver.py","scripts/agent-control/state_manager.py","scripts/check_agent_handoff.py","scripts/session_context.py","tests/test_agent_local_loop.py","tests/test_agent_plan_lane.py","tests/test_agent_plan_promotion.py","tests/test_agent_route_driver.py","tests/test_session_context.py","tools/test_check_agent_handoff.py"],"authority_consumption_allowed":false,"dispatch_lane":"provider_free_repository_maintenance","expected_artifacts":["One exact-head reviewed route-automation implementation","One full end-to-end packet-boundary crossing whose successor was compiled, not manually authored","Exactly-one promotion and bounded escalation receipts bound to the ledger with zero Provider target merge credential authority or T3 effect","Focused and applicable full verification results"],"external_effect_limit":0,"forbidden_changes":["Do not create a second Plan Execution Ledger, Issue, store, controller, scheduler, queue, lease, or routing owner.","Do not give a child merge credentials, GitHub authority, Provider credentials, or T3 authority.","Do not call a Provider, read credentials, write a target, auto-merge, release, deploy, execute an EFFECT, or handle T3 automatically.","Do not promote or execute a successor from this packet; do not let model text or self-report advance routing state.","Do not silently edit the docs/FUTURE_ROUTE.md inventory manifest or packet sketches; manifest changes are planning diffs only.","Do not change product runtime, schema, evaluator, budget, authority, or branch protection."],"forbidden_next_actions":["Do not activate successor execution, EFFECT execution, automatic T3 handling, or PREFLIGHT from this packet.","Do not bypass Draft/Ready discipline, exact-head review, canonical CI, or merge eligibility.","Do not treat missing, conflicting, stale, or outcome-unknown routing or receipts as success."],"goal":"Wire the missing route-level automation: automatic successor-contract compilation from docs/FUTURE_ROUTE.md at promotion time, the canonical closeout/promotion PR lifecycle, and a continuous route driver repeating across packet boundaries without manual successor authoring.","known_store_mutations":[],"ordered_steps":["Parse the checked docs/FUTURE_ROUTE.md inventory manifest and select exactly one eligible successor with all prerequisites COMPLETE, failing closed on zero, multiple, or stale.","Compile the refreshed twelve-field contract and weak-agent-dispatch capsule against accepted main; never execute the successor.","Integrate compilation into promote_plan: successor_absent and successor_still_current compile and record promotion instead of only escalating; bounded pause escalation remains for genuine DECISION_REQUIRED or blocked routing.","Drive the canonical closeout/promotion PR lifecycle: docs-only closeout diff, Draft PR, checks, review, Ready, canonical CI, eligible squash merge, refresh main.","Repeat across packet boundaries with idempotent state and bounded pauses; stop before EFFECT, PREFLIGHT, or T3 authority.","Run focused applicable full handoff security exact-head review and canonical CI checks."],"packet_id":"PE7-ROUTE-AUTOMATION-1","packet_state":"READY_FOR_EXECUTION","pause_gates":["Stop on unprovable routing, missing or conflicting closeout receipt, promotion of zero or multiple successors, or inventory manifest mismatch.","Stop on any Provider, target, credential, authority, successor execution, EFFECT, or T3 path.","Stop on CI failed, queued, or missing; review absent or stale; or ineligible merge."],"plan_lane_state":"plan_lane_active","private_paths_allowed":false,"prerequisite_receipts":["PE7-SUCCESSOR-PROMOTION-ESCALATION-1 COMPLETE: PR #387 exact head 5fe3d55af19aa7a081115637f8f8a7aa63b581af; merge 597f90282fb6ca72472b890b825684bf54486709; canonical workflow 31403849100"],"prerequisites":["PE7-SUCCESSOR-PROMOTION-ESCALATION-1"],"read_paths":["START_HERE.md","AGENTS.md","docs/CURRENT_STATUS.md","docs/NEXT_DECISION.md","docs/FUTURE_ROUTE.md","docs/MODULE_MAP.md","docs/ARCHITECTURE_BOOK.md","docs/REAL_WORLD_TESTING_PLAYBOOK.md","scripts/session_context.py","scripts/agent-control/"],"rollback":"Revert the route-automation implementation and capsule synchronization; retain authoritative ledger closeout and routing receipts; no external effect is created by this packet.","schema_version":"weak_agent_dispatch.v1","secret_values_allowed":false,"verification":["Focused and applicable full Python/control-plane suites","PYTHONPATH=src uv run --no-project python -m unittest tools.test_check_agent_handoff","uv run --no-project python scripts/check_agent_handoff.py","uv run --no-project python tools/check_security_baseline.py","git diff --check"]}
-->
