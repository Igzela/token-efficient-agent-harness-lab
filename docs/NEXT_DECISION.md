...
| Architecture refactor (R-series) | R1 complete: http_server split. R2 complete: LocalProductStore split. R3 complete: task_analyzer split. R4 complete: dag_manager split. R5 complete: context_pack split. R6 complete: model_profiles split. Next candidate: R7 (concurrency.rs). Each refactor preserves all public behavior and tests. |
