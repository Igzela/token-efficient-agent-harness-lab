...
**Stop:** Reviewer independence, sensitive-evidence handling, environment reconstruction, cost completeness, or drift observation is required but cannot be proved from an existing owner.
...
| provider requests, input/output tokens, latency, resolved model, provider/protocol/endpoint, monetary cost, price source/version | `engine/src/provider/managed_deepseek.rs`, `managed_deepseek_executor.rs`, `engine/src/execution_usage/mod.rs`, and the store-owned journal in `engine/src/storage/local_product_store/managed_acceptance.rs` | missing, conflicting, or untrusted usage/cost/model/endpoint remains unavailable, never zero or requested-model inference |
| task/cell reviewer acceptance, reviewer identity class, objection/disagreement, `review_evidence_ref` | `engine/src/provider/managed_deepseek_executor.rs` `managed_deepseek_review_receipt.v1`; `engine/src/storage/local_product_store/managed_acceptance.rs::managed_deepseek_stage_receipt`; `product_tasks.rs::approve_delegated_product_task`; RWE terminal refs from `live_baseline_coordinator.rs` | missing receipt, non-accepted status, or any material objection is unavailable/fail-closed; this is separate from the PR exact-head review receipt |
| review duration | `LocalProductStore` workflow/node `started_at` and `completed_at` records under `engine/src/storage/local_product_store/managed_acceptance.rs` | absent or non-monotonic timestamps make `review_minutes` unavailable and block completeness |
| provider/agent/review/rework/CI/recovery cost fields | `implementation_cost_receipt.v1` validation in `engine/src/rwe/economic_protocol.rs`, provider journal/usage above, GitHub PR/Actions and existing route/review ledger owners | each unrecorded field stays unavailable; failed and outcome-unknown attempts retain known consumed cost |
...
| runner toolchain and dependency-lockfile digests | accepted checkout lockfiles plus the existing RWE snapshot/artifact owner; no current baseline receipt is assumed | until the snapshot owner emits exact values, these fields are unavailable and preflight stops |
| redacted public evidence and restricted raw access/retention/deletion receipts | redaction and content-exclusion paths in `engine/src/storage/local_product_store/product_tasks.rs`; existing `LocalProductStore` artifact/evidence owner | no raw public content; no new run without an owner-issued access/retention/deletion receipt; historical digests are preserved and this packet never deletes or copies bundles |
...
4. **Reviewer policy.** The accepted task/cell policy is `reviewer_identity_class=operator`, `minimum_reviewers=1`, `blinded=false`, `permitted_repair=one bounded repair cycle`, `measure_review_time=true`, and `disagreement_resolution=record disagreement and fail closed`, bound to rubric sha256 `0e3c4275aacae5ae1eec563ea348135fa05b6719391c526490a7503b497c4e7b`. Task/cell reviewer acceptance uses the `managed_deepseek_review_receipt.v1` owner mapped above; the PR exact-head receipt is only repository merge evidence and never substitutes for task/cell review evidence. Review transcripts and raw prompts/outputs are not retained in this manifest.
...
6. **Lifecycle-cost completeness.** The required fields are `provider_requests`, `input_tokens`, `output_tokens`, `latency_ms`, `monetary_cost`, `agent_sessions`, `review_cycles`, `repair_iterations`, `ci_runs`, `ci_compute_minutes`, `human_preparation_minutes`, `review_minutes`, `material_rework_minutes`, and `recovery_minutes`. Failed, cancelled, timed-out, killed, recovered, and outcome-unknown attempts retain consumed cost; unknown fields remain unavailable rather than zero. Runtime usage stays with usage/store owners and engineering cost stays a bounded board report/`implementation_cost_receipt.v1`; this packet creates no cost ledger.
7. **Harness reconstruction.** The old/pre-AC reconstruction set is the exact accepted pre-convergence Harness main SHA, versioned RWE artifacts, task source/tree and definitions, tracked configuration, dependency lockfiles, executor/model/provider profile, runner/toolchain identity, workflow/CI identity, store schema marker, and existing artifact/terminal evidence references. A missing or conflicting component means `reconstructable=false` and stops snapshot/preflight; it does not authorize a guessed environment or a second artifact owner.
8. **Restricted and redacted evidence.** Never commit, print, or put in public comments raw prompts, model outputs, transcripts, credentials, private paths, or unredacted repository content. The public evidence surface is the content-excluded/redacted `product_task_terminal_evidence.v2` projection plus digest-only references. Restricted bundles remain with the existing `LocalProductStore` artifact/evidence owner; access is owner-mediated, retention is receipt-bound, and deletion is owner-mediated only after the recorded retention obligation. This route performs no copy or deletion and does not invent a TTL or access actor. Until the owner supplies access/retention/deletion receipts for a new baseline, those fields are `unavailable` and preflight stops; historical bundle digests remain preserved.
