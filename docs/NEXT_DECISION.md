...
```text
[window: PE7-AC0-DATA-CONTRACT-INVENTORY-1 — DECISION_REQUIRED, runtime inventory is accepted and the next provider-free contract inventory is ready for promotion]

→ `PE7-AC0-DATA-CONTRACT-INVENTORY-1` — promote only after refreshing current-main owners and issuing a new bounded dispatch capsule
```
...
**Prerequisite:** `PE7-AC0-RUNTIME-INVENTORY-1` — COMPLETE; the accepted provider-free inventory classifies the current runtime/executor families and records the benchmark-only timeout exception as a future migration boundary.

**Class:** `CONTRACT`

**Outcome:** Enumerate Golden Path responsibilities, LocalProductStore transaction entries, wire/codegen/SDK/Dashboard projections, configuration construction, and legacy abstractions without changing behavior.

**Allowed delta:** Provider-free inventory and contract evidence only; after promotion update only `docs/CURRENT_STATUS.md` and `docs/NEXT_DECISION.md`; no transaction-view, schema, composition-root, replacement-module, provider, target, or external-effect change.

**Owner/seam:** Reuse `engine/src/product_golden_path.rs`, `engine/src/storage/local_product_store/`, `engine/src/wire_types.rs`, `wire_contract/`, `codegen/`, `sdk/`, `dashboard/`, and the existing Rust composition root; add no second store, runtime, schema, SDK, Dashboard, scheduler, evaluator, or authority owner.

**Exit:** One exact owner/caller/transaction/projection/legacy matrix with compatibility, failure, and rollback obligations for AC1–AC7; no unclassified current caller remains.

**Stop:** A current owner or legacy caller is ambiguous, SQLite/PostgreSQL behavior cannot be mapped, a projection is secretly authoritative, or inventory requires a behavior/schema/authority change. Return `DECISION_REQUIRED` and preserve the partial evidence.

**Rollback:** Revert only the inventory and packet-status document edits; retain the accepted runtime inventory and all prior DB failure evidence. Do not begin AC1 or replay the parked DB run.

**Next permitted action:** Refresh the listed paths against accepted `main`, expand this packet into a new machine-bound dispatch capsule, and independently review that promotion before execution.
