...
| 4. Dead Code Cleanup | Remove `app_layer/` (fully duplicated); remove `dispatch/manual/` (superseded); remove `harness/{sandbox,supervisor,batch_runner,sampling,model_eval}`; remove `storage/{durable_store,health_checker,storage_migrator}`; tag `event_source/`+`errors`+`event_schema` as reference-only | Next |
