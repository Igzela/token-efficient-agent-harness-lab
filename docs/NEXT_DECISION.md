...
| task/cell reviewer acceptance, reviewer identity class, objection/disagreement, `review_evidence_ref` | **`UNAVAILABLE_NOW`: no accepted task/cell reviewer-evidence owner exists.** `managed_deepseek_review_receipt.v1` at `engine/src/provider/managed_deepseek_executor.rs` and its `managed_acceptance.rs` stage-reader prove only status, objection count/hash, and resolved model; terminal refs and PR review are not task/cell reviewer evidence | `reviewer_acceptance_rate`, reviewer identity, explicit disagreement receipt, and `review_evidence_ref` remain unavailable; RWE preflight and observation completeness stop |
...
| provider requests, input/output tokens, latency, resolved model, endpoint, monetary cost, price source/version | `engine/src/provider/managed_deepseek.rs`, `managed_deepseek_executor.rs`, `engine/src/execution_usage/mod.rs`, and the store journal in `engine/src/storage/local_product_store/managed_acceptance.rs` | a missing per-cell receipt is `UNAVAILABLE_NOW`; known consumed cost is retained, and unknown is never zero or requested-model inference |
