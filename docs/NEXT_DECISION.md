...
This document is the single owner of forward routing, packet prerequisites, entry and exit gates, and the immediate next permitted action. Accepted historical facts belong in `docs/CURRENT_STATUS.md`; durable architecture and authority invariants belong in `docs/ARCHITECTURE_BOOK.md`; current owners belong in `docs/MODULE_MAP.md`.
...
The route is stage-ordered but executed through the micro-packets defined below:
...
2. Every later packet defined in this document — `BLOCKED_PREREQUISITE` on its explicit predecessor and, for `EFFECT` packets, separate current operator authority.
...
Only `PE7-RWE-V2-REFREEZE-1` is execution-ready now. Every later micro-packet is routing-only until its exact predecessor is accepted and its local contract fields are refreshed against then-current `main`. An implementation agent must stop `DECISION_REQUIRED` rather than fill a missing architecture, authority, statistical, evaluator, retention, spend, recovery, or adoption decision.
...
An execution session should not load the entire future route. It reads `START_HERE.md`, the current status, the common contracts/hard stops in this document, the one active packet block, its accepted predecessor receipt, the relevant owner map/architecture sections, and the exact code/tests. Other future packet blocks are routing context only.
...
After merge, `PE7-RWE-V2-VIABILITY-PREFLIGHT-1` becomes the next packet. Merge does not itself authorize a live run; `PE7-RWE-V2-VIABILITY-RUN-1` remains separately blocked.
