...
1. `PE7-PRODUCT-GOLDEN-PATH-RESIDUAL-SEAL-2` — `IN_PROGRESS` (PRs #268–#277 are merged and disposable fixture Draft PR acceptance passed; mandatory managed-executor E2E through Draft PR output remains).
...
4. **Code prerequisites merged via PRs #275 and #276; exact Claude admission is the focused final implementation.** The admitted managed executor receives the exact persisted objective only in its in-memory leased input; public task/plan/terminal evidence stays redacted. Its `product_apply_binding.v2` additionally binds exact binary/version/hash/model/context/output and every standard/cache pricing dimension with source/check date, plus task token/call/retry/concurrency budgets. Claude Code 2.1.217 is admitted only with the pinned Haiku snapshot, `--max-turns 3`, a pre-reserved 792,000-token conservative ceiling, and a $2.16 worst-case client-estimate cap covering the higher one-hour cache-write price across all admitted turns. It uses safe mode, no session persistence, strict empty MCP, worktree-bounded Read plus path-scoped Edit/Write, and Bash/network/subagent denial. Its child environment rejects proxy, alternate-model, cloud-routing, and TLS/proxy overrides; only bounded process variables plus explicitly allowlisted first-party OAuth/API credentials may pass. Missing or changed identity, fewer than 792,000 authorized tokens, more than one scheduler call, any retry, or concurrency above one fails before the model process. Usage is owner reported, but CLI dollar fields are client-side estimates rather than authoritative billing and canonical cost evidence stays unavailable. This execution authorization never grants artifact approval or output confirmation. The deterministic fake-binary contract test exercises routing and evidence shape only and is not managed coding-agent acceptance evidence. The audited local first-party OAuth identity currently has no access to the admitted Haiku snapshot (the CLI returned zero tokens and zero cost); configured proxy `mimo-*` models are not substitutes because no authoritative context/pricing contract is admitted. Merge the default-off implementation after exact-head CI/review, then retain the packet as incomplete until admitted model access exists and one disposable live Claude → verification → independent approval → Draft PR acceptance passes. Rollback disables Claude's separate gate and registration; no schema downgrade is required.
