...
**All phases complete (2026-06-08):**

| Order | Phase | Status |
|---|---|---|
| HA-1 | Scheduler Resilience + Persistent Heartbeat | **DONE** — `catch_unwind` wrapper in scheduler tick, persistent heartbeat to `scheduler_heartbeat` SQLite table (singleton-row pattern), auto-restart with exponential backoff on panic (1s–30s), `panic_count`/`restart_count` in status, health endpoint reads persisted heartbeat as fallback on restart, `ACP_HEARTBEAT_INTERVAL_SEC` (default 10s), 6 new tests |
| HA-2 | Automated Backup + Retention | **DONE** — Timer-based backup in scheduler loop (`ACP_BACKUP_INTERVAL_SEC`, default 0=disabled), WAL checkpoint before copy, `prune_backups` with retention policy (`ACP_BACKUP_RETAIN_COUNT`, default 5), `backup_stats()` method, backup metadata in metrics endpoint |
| HA-3 | Deep Health + Resource Monitoring + External Monitoring | **DONE** — `resource_monitor` module with `disk_usage()` (libc statvfs), `memory_usage()` (/proc/meminfo), `db_file_size()`. Health endpoint returns structured JSON with `db`, `disk`, `memory`, `scheduler`, `backup` checks. Degraded on low disk (<10%), stale scheduler, or missing backup. Unhealthy on DB unreachable. Optional webhook alert via `ACP_HEALTH_ALERT_WEBHOOK_URL`. `ACP_DISK_WARN_PCT` and `ACP_MEM_WARN_PCT` thresholds. |
| ~~HA-4~~ | ~~Circuit Breaker for Providers~~ | **DONE** — `CircuitBreaker` module with 14 tests, `CircuitBreakerProvider` wrapper, `CircuitBreakerRegistry`, status endpoint |
| HA-5 | TLS Inbound | **DONE** — `axum_server::bind_rustls()` with `ACP_TLS_CERT_PATH`/`ACP_TLS_KEY_PATH`, plain TCP fallback when absent, graceful shutdown preserved in both paths, `tls_enabled` field in server state. Single-sided TLS env (only cert or only key) now fatal exits on startup. |
| HA-6 | Secret Encryption at Rest | **DONE** — `rusqlite` `bundled-sqlcipher` feature, `new_with_encryption()` constructor, `ACP_DB_ENCRYPTION_KEY` env var, `encryption_active` field, `encryption_key_hash` in backup metadata for restore validation, backward-compatible with unencrypted databases |
