...
5. **Only semantic delta.** Replace direct success/exit interpretation in the enumerated callers with `ProcessBoundaryMapping`; preserve existing wire evidence and lifecycle status names.
...
8. **Failure and recovery.** `ProcessEffectState::Unknown` or `ProcessOutcomeState::Unknown` remains non-success and non-retryable; preserve existing outcome-unknown, lease, rollback, and reconciliation owners; revert the caller-only head to return to the accepted boundary core.
