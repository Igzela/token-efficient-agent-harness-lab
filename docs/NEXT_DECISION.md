...
```text
[window: Plan-packet lifecycle controller — READY_FOR_EXECUTION, provider-free control-plane implementation only]
...
1. `PE7-LIFECYCLE-CONTROLLER-1` — `READY_FOR_EXECUTION`. It is the sole current packet and wires plan-packet CI/review/merge/closeout receipt transitions through the existing owners, ledger-anchored and controller-owned. It creates no Provider, target, or T3 effect.
...
No packet other than the current one is execution-ready. `PE7-LIFECYCLE-CONTROLLER-1` is execution-ready below; `PE7-PLAN-LANE-ACTIVATION-1` is complete; `PE7-RWE-V2-VIABILITY-PREFLIGHT-1` is blocked pending the control-plane transition's later B1/B2/provenance reconciliation; every packet in `docs/FUTURE_ROUTE.md` is routing-only until its exact predecessor is accepted and its complete contract is moved here and refreshed against then-current `main`. An implementation agent must stop `DECISION_REQUIRED` rather than fill a missing architecture, authority, statistical, evaluator, retention, spend, recovery, or adoption decision.
...
**Prerequisite:** `PE7-PLAN-LANE-ACTIVATION-1` is COMPLETE on accepted main `e55e19f1b7c353b4baa2b40ee7b5b16af8918a6c`, with PR #382 exact head `dde26f884ce8a85b776b5933c84c4e6cfd73cb19`, exact-head `PASS`, canonical workflow `31395404498`, and merge receipt `e55e19f1b7c353b4baa2b40ee7b5b16af8918a6c`; the Plan Execution Ledger Issue #383 is provisioned and the lane is active.

**Class:** `IMPLEMENT`

**Outcome:** Wire the plan-packet lifecycle transitions — CI receipt, exact-head review receipt, maintainer merge receipt, and canonical closeout receipt — through the existing owners so an admitted plan subject can traverse claim → dispatch → handoff → CI → review → merge → closeout entirely from the ledger plus authoritative GitHub/PR/CI/review state, with no model self-report advancing state and no second owner.

**Allowed delta:** `scripts/agent-control/dispatcher.py`, `local_run_once.py`, `local_loop.py`, `local_supervisor.py`, `plan_lane.py`, `state_manager.py`, `loopctl.py`, `scripts/session_context.py`, `scripts/check_agent_handoff.py`, their focused control-plane tests, and the smallest canonical-document capsule/status synchronization required by this packet.

**Exit:** A plan subject's terminal transitions (CI receipt, review receipt, merge receipt, closeout receipt) are read back from authoritative GitHub state and recorded on the ledger as controller-owned transitions; restart reconstructs the lifecycle idempotently; no child gains merge/GitHub/Provider/T3 authority; successor promotion, EFFECT execution, and automatic T3 handling remain inactive.
...
1. **Outcome and non-goals.** Wire plan-packet terminal lifecycle transitions through the existing CI/review/merge/closeout owners. Do not create a second controller, ledger, store, or state owner; do not activate successor promotion, EFFECT execution, automatic T3 handling, product runtime changes, target writes, Provider calls, releases, deployments, adoption, or branch-protection changes.
2. **Prerequisites and evidence.** Accepted main `e55e19f1b7c353b4baa2b40ee7b5b16af8918a6c`; predecessor receipt is PR #382 exact head `dde26f884ce8a85b776b5933c84c4e6cfd73cb19`, exact-head `PASS`, canonical workflow `31395404498`, merge `e55e19f1b7c353b4baa2b40ee7b5b16af8918a6c`; Plan Execution Ledger Issue #383 provisioned.
3. **Owners and paths.** Sole lifecycle anchor: Plan Execution Ledger via `scripts/agent-control/control_state.py`; controller transitions: `dispatcher.py`, `state_manager.py`; outer loop: `local_loop.py`, `local_supervisor.py`, `loopctl.py`; plan compiler/executor: `plan_lane.py`, `local_run_once.py`; CI owner: existing `ci_verifier.py`/monitor workflow; review owner: existing review loop; merge owner: repository-maintenance maintainer; closeout owner: canonical docs + packet state.
...
5. **Only semantic delta.** Add controller-owned lifecycle transition recording and readback for plan subjects through existing owners; no new semantic plan contract, no second representation.
...
7. **Ordered implementation slices.** (a) record CI receipt on the ledger for plan PRs through the existing CI owner; (b) record exact-head review receipt through the existing review owner; (c) record maintainer merge receipt through the existing merge owner; (d) record canonical closeout receipt and terminal packet state; (e) add idempotent lifecycle readback/recovery from ledger + GitHub/PR/CI/review state; (f) focused/full verification.
8. **Failure, recovery, and stop taxonomy.** Fail closed on missing ledger, missing/conflicting receipt, stale accepted main, duplicate transition, unknown external outcome, or unavailable owner. Restart reconstructs from ledger plus authoritative GitHub/PR/CI/review state, idempotently, and never re-runs a model after a provable handoff.
9. **Verification.** Focused lifecycle transition/readback/recovery tests; session-context and handoff tests; applicable Python/control-plane suite; security baseline; `scripts/check_agent_handoff.py`; `git diff --check`; exact-head canonical CI; complete-diff independent `PASS`.
10. **Compatibility, rollback, and retention.** Existing Issue lane and activation behavior unchanged. Revert only this packet's code/docs; retain ledger and authoritative receipts; do not delete failed, rejected, or outcome-unknown transitions; no schema migration or new persistence owner.
11. **Exit artifact.** Exact-head accepted lifecycle-controller receipt binding the five transition identities (CI, review, merge, closeout, terminal packet state), focused/full verification, and implementation-cost receipt; successor/EFFECT/T3 paths remain explicitly inactive.
12. **Next action.** After this packet is accepted, promote exactly one successor/promotion/escalation packet against refreshed accepted main. Do not activate successor promotion, EFFECT execution, automatic T3 handling, or PREFLIGHT from this packet.

### 11. Weak-Agent Dispatch Capsule

The active capsule below binds this lifecycle-controller packet. Its effect
limit is zero; it grants no Provider, target, merge, GitHub, credential,
authority, or T3 access.

<!-- weak-agent-dispatch:v1
{"allowed_outputs":["Controller-owned plan-packet lifecycle transition bindings and readback evidence only","Focused provider-free lifecycle transition, recovery, and subject-identity verification evidence"],"allowed_paths":["docs/CURRENT_STATUS.md","docs/NEXT_DECISION.md","scripts/agent-control/dispatcher.py","scripts/agent-control/local_loop.py","scripts/agent-control/local_run_once.py","scripts/agent-control/local_supervisor.py","scripts/agent-control/loopctl.py","scripts/agent-control/plan_lane.py","scripts/agent-control/state_manager.py","scripts/check_agent_handoff.py","scripts/session_context.py","tests/test_agent_local_loop.py","tests/test_agent_plan_lane.py","tests/test_session_context.py","tools/test_check_agent_handoff.py"],"authority_consumption_allowed":false,"dispatch_lane":"provider_free_repository_maintenance","expected_artifacts":["One exact-head reviewed lifecycle-controller implementation","Controller-owned CI/review/merge/closeout receipt transitions bound to the ledger with zero Provider target merge credential authority or T3 effect","Focused and applicable full verification results"],"external_effect_limit":0,"forbidden_changes":["Do not create a second Plan Execution Ledger, Issue, store, controller, scheduler, queue, lease, CI owner, review owner, merge owner, or closeout owner.","Do not give a child merge credentials, GitHub authority, Provider credentials, or T3 authority.","Do not call a Provider, read credentials, write a target, auto-merge, release, deploy, promote a successor, execute an EFFECT, or handle T3 automatically.","Do not let model text or self-report advance lifecycle state.","Do not change product runtime, schema, evaluator, budget, authority, or branch protection."],"forbidden_next_actions":["Do not activate successor promotion, EFFECT execution, automatic T3 handling, or PREFLIGHT from this packet.","Do not create a second semantic plan contract when the accepted weak-agent-dispatch:v1/session-context representation is sufficient.","Do not treat missing, conflicting, stale, or outcome-unknown receipts as success."],"goal":"Wire plan-packet CI review merge and closeout receipt transitions through existing owners with controller-owned ledger bindings and idempotent lifecycle recovery.","known_store_mutations":[],"ordered_steps":["Bind and record the CI receipt for a plan PR through the existing CI owner.","Bind and record the exact-head review receipt through the existing review owner.","Bind and record the maintainer merge receipt through the existing merge owner.","Record the canonical closeout receipt and terminal packet state on the ledger.","Add idempotent lifecycle readback and recovery from ledger plus GitHub PR CI and review state.","Run focused applicable full handoff security exact-head review and canonical CI checks; stop before later route packets."],"packet_id":"PE7-LIFECYCLE-CONTROLLER-1","packet_state":"READY_FOR_EXECUTION","pause_gates":["Stop on missing or conflicting ledger/CI/review/merge/closeout receipt.","Stop on any Provider, target, credential, authority, successor, EFFECT, or T3 path."],"plan_lane_state":"plan_lane_active","private_paths_allowed":false,"prerequisite_receipts":["PE7-PLAN-LANE-ACTIVATION-1 COMPLETE: PR #382 exact head dde26f884ce8a85b776b5933c84c4e6cfd73cb19; merge e55e19f1b7c353b4baa2b40ee7b5b16af8918a6c; canonical workflow 31395404498"],"prerequisites":["PE7-PLAN-LANE-ACTIVATION-1"],"read_paths":["START_HERE.md","AGENTS.md","docs/CURRENT_STATUS.md","docs/NEXT_DECISION.md","docs/MODULE_MAP.md","docs/ARCHITECTURE_BOOK.md","docs/REAL_WORLD_TESTING_PLAYBOOK.md","scripts/session_context.py","scripts/agent-control/"],"rollback":"Revert the lifecycle-controller implementation and capsule synchronization; retain authoritative ledger/CI/review/merge/closeout receipts and all failed or outcome-unknown transitions; no external effect is created by this packet.","schema_version":"weak_agent_dispatch.v1","secret_values_allowed":false,"verification":["PYTHONPATH=src uv run --no-project python -m unittest tests.test_agent_plan_lane tests.test_agent_local_loop tests.test_session_context","PYTHONPATH=src uv run --no-project python -m unittest tools.test_check_agent_handoff","uv run --no-project python scripts/check_agent_handoff.py","uv run --no-project python tools/check_security_baseline.py","git diff --check"]}
-->
