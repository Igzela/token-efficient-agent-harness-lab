...
| Architecture refactor (R-series) | R1 complete: http_server split. R2 complete: LocalProductStore split. R3 complete: task_analyzer split. R4 complete: dag_manager split. Next candidate: R5 (app_layer or workflow remaining files). Each refactor preserves all public behavior and tests. |
