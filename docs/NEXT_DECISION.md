...
**Read-only boundary:** This packet guarantees only the SQLite-backed RWE preflight path. It does not make `LocalProductStore` a universally capability-safe read-only API; ProductTask verification, workspace recovery, admission, and effects remain on their existing mutable owners and are not reachable from this preflight path. PostgreSQL evidence is limited to operation-level authentication/preflight metadata nonmutation and does not claim a general PostgreSQL read-only capability. The production opener does not read encryption keys; unencrypted SQLite is supported here, while encrypted SQLCipher preflight returns `encryption_readiness_unavailable` pending a separately authorized redacted owner. General capability hardening requires a separately authorized successor packet.
