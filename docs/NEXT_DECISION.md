...
1. `PE7-RWE-V2-VIABILITY-PREFLIGHT-1` — `READY_FOR_EXECUTION`. Its prerequisite is accepted (PR #370; see the accepted receipts in `docs/CURRENT_STATUS.md`), its twelve-field contract is expanded against current `main`, its two operator-policy values are accepted (B1 preflight-receipt maximum age = 15 minutes; B2 one-use authorization request-package validity window = 2 hours from issuance), and its weak-agent dispatch capsule is authored in this block. A coding session entry may now run the provider-free preflight procedure (S1–S5) within the bound contract.
...
No packet other than the current one is execution-ready. `PE7-RWE-V2-VIABILITY-PREFLIGHT-1` is execution-ready below; every packet in `docs/FUTURE_ROUTE.md` is routing-only until its exact predecessor is accepted and its complete contract is moved here and refreshed against then-current `main`. An implementation agent must stop `DECISION_REQUIRED` rather than fill a missing architecture, authority, statistical, evaluator, retention, spend, recovery, or adoption decision.
...
**Why this window is open:** this packet was promoted out of `docs/FUTURE_ROUTE.md` when its prerequisite closed (removed in PR #375; the last accepted `main` still containing the routing sketch is `9d2e9458`, and the sketch is preserved verbatim below so no agent must mine Git history for accepted routing facts). The twelve-field contract below was expanded to the maximum derivable from accepted `main` `2ee6571e` and is re-verified against current accepted `main` `0a7fde68` (PRs #376/#377 changed only canonical documentation, so all engine evidence identities and frozen artifacts are byte-identical). The two values that blocked execution readiness (blockers B1 and B2) are now accepted as packet-local operator policy in the Resolved Blockers section below, with their accepted owners; the field-11 weak-agent dispatch capsule is authored in this block and binds the complete contract.
...
The `weak-agent-dispatch:v1` capsule below is authored at promotion, matches this contract exactly, and binds packet identity/state, the `provider_free_local` lane, goal, prerequisites, allowed/read paths, ordered steps, forbidden changes and forbidden next actions, verification, expected artifacts, rollback, pause gates, `external_effect_limit=0`, `authority_consumption_allowed=false`, `secret_values_allowed=false`, `private_paths_allowed=false`, and `plan_lane_state="plan_lane_deferred_until_terminal_owners"`. The capsule and the entry it feeds create no authority.
...
12. **Next permitted action and forbidden next actions.** Next permitted: a coding session entry (which now reports `FRESH_PACKET`/`RESUME` with the field-11 capsule bound) runs the provider-free preflight procedure (S1–S5) within this contract; the preflight must reach `ready=true`, zero blockers, exact frozen bindings, all three negative-effect flags `false`, and receipt freshness within the accepted B1 maximum age (15 minutes), and the request package must carry `expires_at` = authenticated issuance time + the accepted B2 window (2 hours). Forbidden next actions: issuing or consuming a one-use authorization; any Provider call; any credential read; any target write; running the four-cell schedule; promoting or activating `PE7-RWE-V2-VIABILITY-RUN-1` or any successor; repairing or rerunning the v2 freeze or calibration; changing v2 corpus/protocol/schedule/budgets/model/verifier/seeds/stop rules; changing production code, schema, store, or persistence authority; starting a later packet or broadening scope.

### Resolved Blockers (packet-local accepted operator-policy values)

Both values below are accepted as **packet-local** operator policy for `PE7-RWE-V2-VIABILITY-PREFLIGHT-1` (operator decision accepted 2026-08-10). Their accepted owner is this packet block in `docs/NEXT_DECISION.md` plus the confirmed-gap truth in `docs/CURRENT_STATUS.md`; they are enforced at execution by S3/S4 and the field-11 dispatch capsule. They must not be generalized to unrelated authorization classes.
...
**Who owns the next step:** a coding session enters the bound `READY_FOR_EXECUTION` window through `session_context.py enter --role coding`, which reports `FRESH_PACKET` with the field-11 capsule, the exact checkpoint commands, and no new authority; the coding agent then runs S1–S5 provider-free. No further planning decision is required before that entry.
