...
7. **Ordered closeout slices.** (1) prove the merged-tree zero-match inventory and preserved separate paths; (2) record the exact cleanup diff cost, complete `implementation_cost_receipt`, and rollback index; (3) record the accepted pre-AC reconstruction manifest and post-AC main/tree/toolchain identities; (4) leave Contemporary RWE reconstruction and all later EFFECT packets blocked until separately promoted.
