...
**Exit:** An independently reviewed `rwe_operations_evidence.v1` manifest that names the existing evidence owner for every required field and makes missing evidence explicitly `unavailable`.
...
4. **Reviewer policy.** The accepted policy is `reviewer_identity_class=operator`, `minimum_reviewers=1`, `blinded=false`, `permitted_repair=one bounded repair cycle`, `measure_review_time=true`, and `disagreement_resolution=record disagreement and fail closed`, bound to rubric sha256 `0e3c4275aacae5ae1eec563ea348135fa05b6719391c526490a7503b497c4e7b`. The existing exact-head review receipt records authenticated identity, independent reviewer-session identity, transport, implementation-session identity when parent-posted, reviewed SHA/range, and outcome. Review transcripts and raw prompts/outputs are not retained in this manifest.
...
8. **Restricted and redacted evidence.** Never commit, print, or put in public comments raw prompts, model outputs, transcripts, credentials, private paths, or unredacted repository content. Existing restricted bundles may be referenced only by digest/classification and remain under their existing access owner; redacted receipts are the public evidence surface. This route performs no copy, deletion, or retention migration, invents no TTL or access actor, and does not rewrite historical bundles. If the existing owner cannot provide an access/deletion/retention receipt when required, the field is `unavailable` and preflight stops.
