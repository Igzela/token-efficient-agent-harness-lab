...
**Allowed delta:** `engine/src/storage/local_product_store/product_tasks.rs`, `engine/src/storage/local_product_store/managed_acceptance.rs`, focused tests for ProductTask verification/managed review, and the two canonical status documents for closeout; reuse `engine/src/node_executor.rs` as the sole mapping owner.
