...
| HA-5 | TLS Inbound | **DONE** — `axum_server::bind_rustls()` with `ACP_TLS_CERT_PATH`/`ACP_TLS_KEY_PATH`, plain TCP fallback when absent, graceful shutdown preserved in both paths, `tls_enabled` field in server state |
