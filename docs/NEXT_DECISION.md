...
1. `PE7-PRODUCT-GOLDEN-PATH-RESIDUAL-SEAL-2` — `IN_PROGRESS` (PRs #268–#276 are merged; disposable Draft PR acceptance and mandatory managed-executor E2E remain).
...
4. **Code prerequisites merged via PRs #275 and #276; final live acceptance remains.** The admitted managed executor must receive the exact persisted objective only in its in-memory leased input; public task/plan/terminal evidence stays redacted. Its `product_apply_binding.v2` binds task/workspace/source/objective/paths/output plus the resolved positive measured-token threshold and call/retry budgets, cumulative failed-attempt usage survives restart, and excess/missing usage fails before verification or output. The token contract must additionally prevent or interrupt a provider call before it exceeds the authorized task total; rejecting overage only after process exit is insufficient. Codex runs non-interactively inside the existing exact `workspace-write` sandbox; this execution authorization never grants artifact approval or output confirmation. The current installed Codex CLI `0.145.0` exposes no task-scoped token-cap option and reports JSONL usage only at `turn.completed`, so this strict acceptance item remains unresolved and live managed output acceptance must not be marked complete. Pass the deterministic path and an executor satisfying that token contract through the admitted managed coding-executor E2E plus disposable Draft PR acceptance, then synchronize SDK/Dashboard facts and active documents before marking the packet complete. Rollback keeps the scrub-aware public readers in place while disabling the v2 writer/in-memory injection path; before removing those readers, a transactional maintenance step must delete `_execution_objective_v1` from every persisted intake. A plain revert to pre-scrub readers is forbidden because it would expose the internal objective. v1 bindings remain readable and the safe rollback requires no schema downgrade.
