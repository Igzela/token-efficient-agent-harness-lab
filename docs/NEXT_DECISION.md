...
| Supervised autonomous beta planning | Planning-only track accepted in ADR-0002. Batch 0-6 governance/module/model/read-only-planner/durable-state/advisory/design-gate work is complete. Batch 7 implementation-plan artifact is documented: the first planned primitive is an app-owned detached patch workspace/snapshot outside registered target repositories, not a registered target `git worktree`. Implementation remains NO-GO: no schema, tests, runtime controls, workers, target writes, sandbox/process/container/VM execution, deploy/merge controls, or default-on provider calls are approved. |
...
| 7 | Supervised execution beta | **NO-GO for implementation**. ADR-0002 now records a documentation-only implementation-plan artifact for app-owned detached patch workspace/snapshot generation outside registered target repositories, but any code requires a separate approved, test-first implementation slice. |

Batch 2 selects `WorkflowGraph` as the canonical planning and persistence model. `DAGState` remains the graph-mutation model for versioned proposals/rollback, and scheduling-local `DagState` remains the concurrency view for file-overlap scheduling. Batch 3 implemented the read-only planner without R8, file splitting, target writes, worker runtime, provider calls, sandbox/process/container/VM execution, or execution controls. Batch 4 persists workflow run/node/edge/event/approval records only as app-owned state; resume/cancel endpoints record metadata and update stored status only, with no worker or execution authority. Batch 5 connects quality/routing/retry/observability into planning decisions only as `advisory` status/block/recommendation metadata. Batch 6 documents future sandbox/workspace/approval/rollback/artifact requirements and threat-model risks. Batch 7 records a documentation-only implementation plan that selects an app-owned detached patch workspace/snapshot for the first patch artifact slice, but it does not implement or authorize execution.

Batch 7 readiness audit outcome:

| Prerequisite | Current evidence | Status |
|---|---|---|
| Isolation primitive selected | ADR-0002 selects app-owned detached patch workspace/snapshot for the first patch artifact slice and rejects registered-target `git worktree add` because it mutates target `.git/worktrees`. No process/container/VM execution primitive is selected. | Design selected; implementation not started |
| Target workspace contract | ADR-0002 defines source revision evidence, app-owned writable workspace paths outside registered target repositories, diff capture, review-blocked state, cleanup, and quarantine semantics. | Design specified; implementation missing |
...
| Artifact capture schema/storage | ADR-0002 defines `supervised_patch_artifact.v1` minimum fields, app-owned storage, redaction, access, and retention expectations. No SQLite/filesystem capture schema is wired. | Design specified; implementation missing |
...
Next safe action: define and approve one narrow Batch 7 implementation slice before code. Recommended first slice is schema/storage plus tests for app-owned detached patch workspace/artifact records, path canonicalization outside registered target repositories, and boundary evidence only. Do not add command execution, target writes, provider calls, registered-target `git worktree add`, push/merge/deploy/apply controls, or runtime workers.
...
- Supervised planning Batch 7 implementation-plan artifact: ADR-0002 documents app-owned detached patch workspace/snapshot as the first planned primitive, future patch-review approval binding, rollback/quarantine expectations, and `supervised_patch_artifact.v1` schema requirements. No implementation exists.
...
Batch 6 design contracts, Batch 7 readiness audit, and the Batch 7 implementation-plan artifact are planning artifacts only. They do not approve any sandbox/process/container/VM implementation, target workspace writer, approval broker wiring, rollback engine, artifact-capture runtime, worker process, provider call, push, merge, deploy, apply, run, or execution control.
