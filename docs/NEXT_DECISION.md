...
| Target workspace contract | Slice A stores source revision evidence, target/workspace canonical paths, lifecycle status, and boundary JSON in app-owned SQLite. Path-boundary tests reject workspace paths inside registered target repositories. It does not create directories or copy files. | Metadata schema/storage implemented; lifecycle runtime missing |
...
| Artifact capture schema/storage | Slice A stores `supervised_patch_artifact.v1` metadata in app-owned SQLite with patch hash, normalized changed files, redaction status, storage refs, export/import, integrity, and stats coverage. It does not create patch files, run redaction, or expose/export artifacts through an approval gate. | Metadata schema/storage implemented; capture runtime missing |
...
Next safe action: define and approve one narrow Batch 7 Slice B before code. Recommended next slice is an evidence-bound patch-review approval/export design or read-only API visibility, but only after deciding gate semantics. Do not add command execution, workspace directory creation, patch generation, target writes, provider calls, registered-target `git worktree add`, push/merge/deploy/apply controls, or runtime workers.
...
- Supervised planning Batch 7 Slice A: app-owned SQLite `supervised_patch_workspaces` and `supervised_patch_artifacts` metadata records, schema v3 migration, path-boundary validation outside registered target repositories, normalized changed-file validation, stats, integrity, and export/import coverage. No HTTP routes, SDK methods, dashboard UI, workspace creation, patch generation, approval/export gate, rollback engine, target writes, workers, provider calls, or execution controls.
...
Batch 6 design contracts, Batch 7 readiness audit, Batch 7 implementation-plan artifact, and Batch 7 Slice A storage metadata do not approve any sandbox/process/container/VM implementation, target workspace writer, approval broker wiring, rollback engine, artifact-capture runtime, worker process, provider call, push, merge, deploy, apply, run, or execution control.
