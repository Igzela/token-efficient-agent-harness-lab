...
1. `PE7-PLAN-LANE-ACTIVATION-1` — `READY_FOR_EXECUTION`. It is the sole current packet and activates Plan subjects through the existing local outer loop, Plan Execution Ledger, CI/review/merge/closeout owners, and fail-closed recovery boundaries. It creates no Provider, target, or T3 effect.
...
No packet other than the current one is execution-ready. `PE7-PLAN-LANE-ACTIVATION-1` is execution-ready below; `PE7-CTRL-ROUTE-CONTRACT-1` is complete; `PE7-RWE-V2-VIABILITY-PREFLIGHT-1` is blocked pending the control-plane transition's later B1/B2/provenance reconciliation; every packet in `docs/FUTURE_ROUTE.md` is routing-only until its exact predecessor is accepted and its complete contract is moved here and refreshed against then-current `main`. An implementation agent must stop `DECISION_REQUIRED` rather than fill a missing architecture, authority, statistical, evaluator, retention, spend, recovery, or adoption decision.
...
## Packet PE7-PLAN-LANE-ACTIVATION-1

**State:** `READY_FOR_EXECUTION`

**Prerequisite:** `PE7-CTRL-ROUTE-CONTRACT-1` is COMPLETE on accepted main `546cabc1ceb98b49b543d0bd90a62fc228e67338`, with PR #380 exact head `e905cf6ec7a989b54e60f913657ca306f33ebf49`, exact-head `PASS`, canonical workflow `31386777810`, and merge receipt `546cabc1ceb98b49b543d0bd90a62fc228e67338`.

**Class:** `IMPLEMENT`

**Outcome:** Activate Plan subjects through the existing local outer loop and existing Plan Execution Ledger. Replace blanket deferred admission only after real terminal-owner readiness checks prove that CI, exact-head review, maintainer merge, and canonical closeout can bind plan subjects without a second controller or state owner.

**Allowed delta:** `scripts/agent-control/control_state.py`, `dispatcher.py`, `local_loop.py`, `local_run_once.py`, `local_supervisor.py`, `plan_lane.py`, `state_manager.py`, `scripts/session_context.py`, `scripts/check_agent_handoff.py`, their focused control-plane tests, and the smallest canonical-document capsule/status synchronization required by this packet.

**Exit:** One exact plan subject can be admitted and handed off through the existing local outer loop only when terminal-owner readiness passes; the ledger and authoritative GitHub/PR/CI/review state reconstruct the same attempt idempotently after restart; all subject identities and receipts remain bound; Plan execution has no child merge/GitHub/Provider/T3 authority; successor promotion, EFFECT execution, and automatic T3 handling remain inactive.

**Stop:** Any second ledger/controller/store/state owner, caller- or model-asserted lifecycle transition, missing or conflicting subject receipt, stale accepted-main identity, unprovable terminal owner, child authority, Provider/target effect, automatic successor/EFFECT/T3 path, or requirement to choose a new schema, evaluator, retention, spend, statistical, or T3 value.

### Twelve-field contract

1. **Outcome and non-goals.** Admit and execute one bounded Plan subject through the existing local outer loop. Do not activate successor promotion, EFFECT execution, automatic T3 handling, product runtime changes, target writes, Provider calls, releases, deployments, adoption, or branch-protection changes.
2. **Prerequisites and evidence.** Accepted main `546cabc1ceb98b49b543d0bd90a62fc228e67338`; predecessor receipt is PR #380 exact head `e905cf6ec7a989b54e60f913657ca306f33ebf49`, exact-head `PASS`, canonical workflow `31386777810`, merge `546cabc1ceb98b49b543d0bd90a62fc228e67338`; existing owners are documented in `docs/MODULE_MAP.md` and `docs/REAL_WORLD_TESTING_PLAYBOOK.md`.
3. **Owners and paths.** Plan packet/status authority: `docs/NEXT_DECISION.md`; accepted truth: `docs/CURRENT_STATUS.md`; session representation: `scripts/session_context.py`; handoff validation: `scripts/check_agent_handoff.py`; sole durable Plan lifecycle anchor: existing Plan Execution Ledger access in `scripts/agent-control/control_state.py`; outer loop: `local_loop.py`, `local_supervisor.py`, `loopctl.py`; plan compiler/executor: `plan_lane.py`, `local_run_once.py`; controller transitions: `dispatcher.py`, `state_manager.py`; existing CI, review, maintainer merge, and closeout owners remain authoritative.
4. **Frozen subject identity and invariants.** Every plan subject binds `packet_id`, accepted-main SHA, task/dispatch digest, ledger transition identity, branch, PR number, exact head SHA, CI receipt, review receipt, merge receipt, and closeout receipt. The GitHub Plan Execution Ledger remains the sole durable plan-packet lifecycle anchor; local state is rebuildable; no model self-report advances state; no child receives merge credentials, GitHub authority, Provider credentials, or T3 authority.
5. **Only semantic delta.** Replace the blanket `plan_lane_deferred_until_terminal_owners` admission with real, fail-closed terminal-owner readiness checks and admit the existing `_run_plan_once_authorized` path through the existing local outer loop. Consume the accepted `weak-agent-dispatch:v1` / session-context representation rather than introducing a second semantic plan contract. The capsule flips to `plan_lane_active` only behind those readiness checks.
6. **Forbidden changes.** No second Issue, ledger, store, scheduler, controller, queue, lease, CI owner, review owner, merge owner, closeout owner, evaluator, schema migration, authority boundary, Provider call, credential read, target write, automatic merge, release, deployment, successor promotion, EFFECT, or automatic T3 handling.
7. **Ordered implementation slices.** (a) prove ledger/CI/review/merge/closeout terminal-owner readiness for plan subjects; (b) consume the accepted weak-agent dispatch capsule and bind packet/main/task/dispatch/ledger identities; (c) admit plan candidates in `local_loop.py`; (d) replace `run_plan_once` deferral and add idempotent ledger/GitHub/PR/CI/review restart recovery; (e) record and read back controller-owned receipt identities without model advancement; (f) run focused/full verification and synchronize the next route only after acceptance.
8. **Failure, recovery, and stop taxonomy.** Fail closed on missing ledger, malformed capsule, stale accepted main, scope conflict, duplicate dispatch/PR, missing or conflicting exact-head CI/review/merge/closeout receipt, expired lease, unknown external outcome, controller/readback failure, or unavailable terminal owner. Restart reconstructs from the ledger plus authoritative GitHub/PR/CI/review state and never re-runs a model after a provable handoff. No retry treats outcome-unknown as success.
9. **Verification.** Focused Plan parser/admission/readiness/claim-recovery/subject-receipt tests; session-context and handoff tests; applicable Python/control-plane suite; security baseline; wire drift; `scripts/check_agent_handoff.py`; `git diff --check`; exact-head canonical CI; complete-diff independent `PASS`.
10. **Compatibility, rollback, and retention.** Existing Issue lane behavior remains unchanged. Disable the active Plan route by reverting this packet's code/docs only; retain ledger and authoritative receipts; do not delete failed, rejected, or outcome-unknown transitions; no schema migration or new persistence owner.
11. **Exit artifact.** Exact-head accepted Plan activation receipt binding packet id, accepted-main SHA, task/dispatch digest, ledger transition identity, branch, PR, exact head, CI/review/merge/closeout receipt identities, focused/full verification, and implementation-cost receipt; successor/EFFECT/T3 paths remain explicitly inactive.
12. **Next action.** After this packet is accepted, promote exactly one lifecycle-controller packet against refreshed accepted main. Do not activate successor promotion, EFFECT execution, automatic T3 handling, or PREFLIGHT from this packet.

### 11. Weak-Agent Dispatch Capsule

The active capsule below binds this Plan-lane activation packet. Its effect limit
is zero; it grants no Provider, target, merge, GitHub, credential, authority, or
T3 access. The lane is active only behind the real terminal-owner readiness
checks implemented by this packet.

<!-- weak-agent-dispatch:v1
{"allowed_outputs":["Bounded Plan subject implementation and controller-owned ledger/receipt bindings only","Focused provider-free Plan admission readiness recovery and subject-identity verification evidence"],"allowed_paths":["docs/CURRENT_STATUS.md","docs/NEXT_DECISION.md","scripts/agent-control/control_state.py","scripts/agent-control/dispatcher.py","scripts/agent-control/local_loop.py","scripts/agent-control/local_run_once.py","scripts/agent-control/local_supervisor.py","scripts/agent-control/plan_lane.py","scripts/agent-control/state_manager.py","scripts/check_agent_handoff.py","scripts/session_context.py","tests/test_agent_local_loop.py","tests/test_agent_plan_lane.py","tests/test_session_context.py","tools/test_check_agent_handoff.py"],"authority_consumption_allowed":false,"dispatch_lane":"provider_free_repository_maintenance","expected_artifacts":["One exact-head reviewed Plan-lane activation implementation","One subject-aware ledger and recovery receipt with zero Provider target merge credential authority or T3 effect","Focused and applicable full verification results"],"external_effect_limit":0,"forbidden_changes":["Do not create a second Plan Execution Ledger, Issue, store, controller, scheduler, queue, lease, CI owner, review owner, merge owner, or closeout owner.","Do not give a child merge credentials, GitHub authority, Provider credentials, or T3 authority.","Do not call a Provider, read credentials, write a target, auto-merge, release, deploy, promote a successor, execute an EFFECT, or handle T3 automatically.","Do not let model text or self-report advance lifecycle state.","Do not change product runtime, schema, evaluator, budget, authority, or branch protection."],"forbidden_next_actions":["Do not activate successor promotion, EFFECT execution, automatic T3 handling, or PREFLIGHT from this packet.","Do not create a second semantic plan contract when the accepted weak-agent-dispatch:v1/session-context representation is sufficient.","Do not treat missing, conflicting, stale, or outcome-unknown receipts as success."],"goal":"Activate one bounded Plan subject through existing queue lease local-loop ledger CI review merge and closeout owners with explicit subject identity and idempotent restart recovery.","known_store_mutations":[],"ordered_steps":["Verify the existing Plan Execution Ledger and exact terminal-owner readiness paths for plan subjects.","Consume and validate the accepted weak-agent-dispatch:v1/session-context representation and bind packet/main/task-dispatch/ledger identities.","Admit Plan subjects in the existing local outer loop with scope and capacity checks.","Replace the blanket Plan deferral with readiness-gated execution and ledger/GitHub/PR/CI/review recovery.","Record and read back controller-owned branch/PR/head/CI/review/merge/closeout receipt identities without model advancement.","Run focused applicable full handoff security exact-head review and canonical CI checks; stop before later route packets."],"packet_id":"PE7-PLAN-LANE-ACTIVATION-1","packet_state":"READY_FOR_EXECUTION","pause_gates":["Stop on missing or conflicting ledger/CI/review/merge/closeout receipt.","Stop on any Provider, target, credential, authority, successor, EFFECT, or T3 path."],"plan_lane_state":"plan_lane_active","private_paths_allowed":false,"prerequisite_receipts":["PE7-CTRL-ROUTE-CONTRACT-1 COMPLETE: PR #380 exact head e905cf6ec7a989b54e60f913657ca306f33ebf49; merge 546cabc1ceb98b49b543d0bd90a62fc228e67338; canonical workflow 31386777810"],"prerequisites":["PE7-CTRL-ROUTE-CONTRACT-1"],"read_paths":["START_HERE.md","AGENTS.md","docs/CURRENT_STATUS.md","docs/NEXT_DECISION.md","docs/MODULE_MAP.md","docs/ARCHITECTURE_BOOK.md","docs/REAL_WORLD_TESTING_PLAYBOOK.md","scripts/session_context.py","scripts/agent-control/"],"rollback":"Revert the activation packet implementation and capsule synchronization; retain authoritative ledger/CI/review/merge/closeout receipts and all failed or outcome-unknown transitions; no external effect is created by this packet.","schema_version":"weak_agent_dispatch.v1","secret_values_allowed":false,"verification":["PYTHONPATH=src uv run --no-project python -m unittest tests.test_agent_plan_lane tests.test_agent_local_loop tests.test_session_context","PYTHONPATH=src uv run --no-project python -m unittest tools.test_check_agent_handoff","uv run --no-project python scripts/check_agent_handoff.py","uv run --no-project python tools/check_security_baseline.py","git diff --check"]}
-->
