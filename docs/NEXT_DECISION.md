...
**Prerequisite:** Accepted main `aa83ac1f5eada74199e0ce28ecb91d37a48769d6` contains the accepted route, control-binding, and SQLite race-repair receipts. The failed fresh route attempt `ea64fd6d-fb8e-5c54-b86c-ae8f96c17550` and the exact manual reproduction both prove HTTP 422 before workflow creation: `agent-controller.yml` declares 29 `workflow_dispatch` inputs while GitHub permits at most 25. No route claim, PR, Provider call, target write, or effect occurred.
...
5. **Only semantic delta.** Replace the 14 route-receipt-only workflow inputs with one bounded versioned `route_payload` input used only by `record-route-t3-receipt` and `record-route-owner-outcome`; decode it without shell evaluation and delegate to the existing validated functions. Other commands and fields retain their current contract.
