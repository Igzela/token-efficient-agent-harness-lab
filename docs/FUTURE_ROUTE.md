...
The AC2 contract and caller migration, AC3 Golden Path split, AC4 transaction views, AC5 composition root, and AC6 schema convergence/compatibility closeout are accepted on `main`; AC1 shared supervision remains deferred optional hardening. The AC7 removal manifest is accepted on `main`; `PE7-AC7-CLEANUP-1` is the current provider-free deletion-only window in `docs/NEXT_DECISION.md`, and the remaining closeout sketch below remains blocked until cleanup is accepted.
