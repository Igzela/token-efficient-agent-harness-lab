...
| Ledger-Orchestrated Harness Candidate (`H_ledger`) | Provider-independent candidate `ledger-orchestrated:provider-independent:v1` implemented and verified under `engine/src/harness_evolution/ledger_orchestration.rs` with deterministic tests (decomposition, self-repair, no-progress guard, truncation recovery, state bounds, cell isolation, fresh-context projection, adapter normalization). Default-off; strictly preserves `Harness × Model × Strategy` matrix factorization. Live evaluation remains pending lower-rung resolution. | Candidate Harness implementation; unexecuted live cells remain unresolved without scientific claims. |
