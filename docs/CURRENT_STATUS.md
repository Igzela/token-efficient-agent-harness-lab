...
- Issue #208 currently has only `agent-control` and `agent-emergency-stop`; orchestration and auto-merge enable labels are absent. The unique Vader runner is registered with the required labels but is offline and idle. Its active local user service runs the current official `2.335.1` listener, but the repository readiness checker still returns `runner_offline` after one safe service restart.
...
The demonstrated #217 failure cause is established from Actions diagnostics. The repository Actions setting and least-privilege administration-read credential prerequisite are now present, but the Vader runner remains offline. Its active current-version local service reproduces TLS transport EOF during token exchange to `tokenghub.actions.githubusercontent.com`; a credential-free host probe to that host times out while the broker host completes TLS. A safe service restart did not restore readiness, so no safe local configuration repair is available. The required operator action is to restore outbound TLS connectivity from the runner host to `tokenghub.actions.githubusercontent.com:443` without an intercepting or terminating middlebox, then rerun the bounded readiness checker. A repaired preflight records bounded terminal evidence, but a replacement smoke has not run and Issue #208 was never enabled during this acceptance attempt.
...
| Durable cross-run memory | Versioned scoped memory, bounded retrieval, runtime context injection, audit, API/SDK, backup, and SQLite/PostgreSQL parity are connected; provider live-safety hardening merged in PR #224, while live embedding remains fail-closed because the current catalog does not admit the fixed embedding identity or complete pricing evidence |
...
7. Controlled external acceptance — staging drills and disposable target repository passed; provider embedding/benchmark are blocked because the current catalog omits the fixed embedding identity and complete modeled pricing; replacement Vader smoke is blocked by the token-exchange TLS path leaving the runner offline; disposable repository deletion awaits GitHub sudo-mode 2FA.
