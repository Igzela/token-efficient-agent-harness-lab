...
- Branch: `main` at CLI pilot stub hardening merge `446cbf7`.
- Tests: full Rust + TypeScript stack verification, 52 Python SDK tests, 154 HTTP tests, focused trusted-local/adaptive tests, rustfmt, Clippy, handoff, wire drift, dashboard boundary lint, and `git diff --check` passed with 0 failures on 2026-06-22. The supervised patch CLI repair test and Trial 4/5 CLI pilot stubs were hardened for noexec `/tmp` environments by keeping fake CLI executables under `target/`.
