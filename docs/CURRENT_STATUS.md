...
| AC2 typed execution contract | `IN_PROGRESS` | Provider-free typed state/outcome/usage contract and executor mappings are recorded below; merge-backed closeout and boundary-core promotion remain pending; no runtime, wire, or schema change is authorized |
...
The dependency order is frozen as `AC0 inventory → AC0 trace/order closeout → AC2 contract → AC2 typed boundary → AC2 caller migration → AC3 → AC4 → AC5 → AC6 → AC7`. AC1 shared `ProcessSupervisor` remains deferred optional hardening and is not a prerequisite. The file-level AC2 contract starts from `engine/src/node_executor.rs::ProcessOutcome`, `engine/src/executor_adapter.rs::ExecutionResult`, `engine/src/provider/executor.rs`, `engine/src/provider/managed_deepseek_executor.rs`, `engine/src/execution_usage/reconcile.rs`, and the existing persistence owners `engine/src/storage/local_product_store/product_tasks.rs` and `engine/src/storage/local_product_store/workflow_runs.rs`; AC2 may add only typed mappings and focused tests until its contract is accepted. It must not move admission, lease, spend, verification, approval, output, audit, recovery, or target authority. Within the enumerated Golden Path and legacy/advisory caller closure, no unknown production caller was found; any caller outside that closure or any cross-backend parity requirement stops the next packet rather than widening scope.
...
| Known outcome | `known_success` is `ProcessOutcome::exited(0)` plus the existing verification/terminal owner for OS-process executors, or an explicit completed provider/managed response with accepted executor evidence; `known_failure` is a definitive non-zero `exited`, `signaled`, `output_read_failed`, or pre-send/definitive adapter refusal | `ProcessOutcome` describes the executor/process observation and is optional for provider/managed responses. A provider response is not forced through an OS-process variant. `signaled` with an unavailable signal remains a known termination with incomplete detail; `unavailable` is incomplete process evidence, never success. `ProductTaskStatus::Completed` is the only canonical product success. |
...
| Usage completeness | `complete`, `partial`, `ambiguous`, `conflicting` are exactly `EventCompleteness::{Complete,Partial,Ambiguous,Conflicting}` | `reconcile_usage_events` may suppress agreeing duplicates, but `ReconcileResult.conflicts` and `EventCompleteness::Conflicting` make `admission_evidence_ok` fail closed. Missing cost is represented by `CostSource::Unavailable`, not fabricated zero cost. |
...
|---|---|---|
| `CommandNodeExecutor` and other local process executors | `ExitStatus` becomes `ProcessOutcome::exited`, `signaled`, or the existing `output_read_failed`; `ProcessOutcome::successful_exit()` is the narrow process-success predicate | Process cleanup, timeout, kill/reap, and output redaction remain executor-owned. A process outcome is not by itself ProductTask terminalization. |
...
| Adaptive provider adapter | `adaptive_plan_missing`, `adaptive_plan_invalid`, `adaptive_policy_*`, `adaptive_request_invalid`, `adaptive_*_limit_invalid`, `adaptive_endpoint_*`, and `adaptive_execution_disabled` are pre-effect `effect_not_started` + definitive failure. `adaptive_execution_killed` is `cancelled`, with effect-started determined by whether call evidence exists. `adaptive_execution_timeout`, `adaptive_call_limit_exceeded`, `adaptive_cost_limit_exceeded`, `adaptive_token_limit_exceeded`, `adaptive_provider_error`, `adaptive_provider_disabled` after a call, `adaptive_provider_token_invalid`, `adaptive_provider_identity_mismatch`, `adaptive_provider_token_over_reservation`, `adaptive_provider_cost_over_reservation`, `adaptive_runtime_failure`, `adaptive_panel_quorum_not_met`, `adaptive_panel_partial_failure`, and `adaptive_fallback_exhausted` are effect-started when call evidence exists; they are `known_failure` only with definitive pre-send/transport evidence, otherwise `unknown`. `AdaptiveExecutionResult` is `known_success` only after its explicit result and existing verification owner accept it. | Adaptive execution does not become a second scheduler, budget ledger, usage owner, or ProductTask authority. An error code alone cannot turn a possibly sent provider call into a retryable known failure. |
...
The focused current evidence is `engine/tests/test_product_golden_path_g1.rs`, `test_product_golden_path_g2.rs`, `test_product_golden_path_g3.rs`, `test_product_golden_path_recovery.rs`, `test_product_golden_path_evidence.rs`, the provider failure/reconciliation tests, and the existing `execution_usage` unit tests. Direct enum/mapping coverage that is still absent is an explicit exit obligation for `PE7-AC2-BOUNDARY-CORE-1`; it does not authorize guessing or widening this contract.
