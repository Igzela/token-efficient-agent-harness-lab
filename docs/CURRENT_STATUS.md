...
- PR #346 is merged and accepted as `adcb87b4a3ece961a46455117ae4323b4f54c2fa` from exact head `64d45f5e55f4393737c26b26dbd66976b8145d5d`: Packet B follow-up repair for PostgreSQL product-task CI failures. It wraps store/resolver key operations in `spawn_blocking` (fixing the runtime-from-runtime panic in PG restart recovery), orders durable store commit before resolver registration/removal so create/rotate/delete fail closed on unknown durable outcomes, re-verifies bootstrap delegation authority durably, and isolates the PG bootstrap/pre-admission/delegated-manifest tests behind gate/env restore with real-revision `docs/USER_GUIDE.md` fixtures. Canonical exact-head run `30732749013` passed; PG suite 73/73 and SQLite serial suite 2680 passed locally; exact-head review receipt posted on `64d45f5e`; no provider call or target effect occurred.
