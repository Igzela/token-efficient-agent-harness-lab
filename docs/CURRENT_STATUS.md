...
|---|---|---|---|
| Intake and Golden Path | `engine/src/product_golden_path.rs` (`validate_intake`, `ValidatedProductTaskIntake`, `ProductTaskStatus`); `engine/src/http_server/handlers/product_tasks.rs` (`api_create_product_task`); Dashboard `MissionControl.submitProductGoldenPath` through the typed SDK client | Intake contract/hash, task version, run/node identity, and redacted public/task projections remain bound to the existing ProductTask | Reject before admission on invalid gates; retain the existing task version and never replay an unproved effect |
| Runtime orchestration | Existing `engine/src/scheduler.rs`, `engine/src/workflow/`, `engine/src/executor_pool.rs`, and `engine/src/node_executor.rs`; ProductTask compile/schedule handlers remain callers | Scheduler leases, node attempts, executor result, verification, and terminal classification stay under the existing runtime/store path | Pause, cancellation, cleanup, failure, and outcome-unknown behavior remains executor/store-owned; no shared ProcessSupervisor is introduced |
| Persistence and transactions | `LocalProductStore` in `engine/src/storage/local_product_store/`, especially `product_tasks.rs` (`admit_product_task`, reservation, delegated preparation, terminal and cleanup paths); SQLite and PostgreSQL backends remain one persistence owner | ProductTask/run/node/attempt, delegation, spend, output, audit, terminal, and recovery rows are committed through the existing transaction boundary | Preserve CAS/idempotency/restart semantics and SQLite/PostgreSQL parity; no direct SQL mutation or second store |
| Provider and credential boundary | Existing managed-provider adapters under `engine/src/provider/` and ProductTask managed-acceptance/store owners; repository maintenance uses the existing OpenCode controller seam | Request journal, usage reconciliation, and redacted evidence are post-boundary records; credentials remain parent-held and never enter persistence or evidence | This packet makes no Provider call and consumes no authority; sending/outcome-unknown remains non-retryable under existing owners |
| Wire, codegen, SDK, Dashboard | `wire_contract/v1/` → `codegen/generate_wire_types.py` → Rust/Python/TypeScript generated outputs; Axum API handlers; `sdk/typescript/`; `dashboard/src/lib/` and existing projection components | Generated types and Dashboard/SDK data are projections of Rust/API contracts, not authorities | No wire/schema migration in AC0; `scripts/check_wire_codegen_drift.sh` remains the drift gate; AC6 owns any later schema migration |
...
The minimum trace set is already exercised by existing tests and remains read-only evidence for the next trace/order freeze:

| Trace anchor | Existing evidence | What it proves |
|---|---|---|
| Intake/admission/status | `engine/tests/test_product_golden_path_g1.rs` and `test_product_golden_path_authority.rs` | Gate rejection, ProductTask status/version transitions, tenant/authority binding, and no bypass through an alternate caller |
| Deterministic execution/restart | `engine/tests/test_product_golden_path_g2.rs` and `test_product_golden_path_recovery.rs` | Bounded fixture execution, workspace/restart recovery, cleanup, and SQLite/PostgreSQL parity paths without a Provider call |
| Delegated/failure evidence | `engine/tests/test_product_golden_path_g3.rs`, `test_product_golden_path_evidence.rs`, and `test_managed_acceptance_delegation.rs` | Store-owned delegation, redaction, failure classification, approval/output sequencing, and fail-closed cleanup |
| Store and API projections | `engine/tests/test_local_product_store.rs`, `engine/tests/test_http_server.rs`, `engine/tests/http_server/`, `sdk/typescript/tests/`, and `dashboard/src/lib/*.test.ts` | Transaction/read projection compatibility without creating a second persistence or UI authority |

The inventory is intentionally not a claim of a successful live DeepSeek run or a decision-grade RWE baseline. Reverting this document-only inventory and the packet-status note is the rollback; existing failure evidence is retained and no parked effect is replayed.
...
4. The AC0 data inventory is in progress as a provider-free contract packet; the trace/order closeout and all AC1–AC7 implementation slices remain blocked until their immediately preceding current-main contract packet is accepted.
