...
|---|---|---|
| `CommandNodeExecutor` and other local process executors | `ExitStatus` becomes `ProcessOutcome::exited` or `signaled`; the existing failure states are `output_read_failed`, `spawn_failed`, `timed_out`, `wait_failed`, `stdout_reader_failed`, `stderr_reader_failed`, `combined_reader_failed`, `process_tree_cleanup_failed`, `output_limit_exceeded`, `process_tree_containment_unavailable`, and `invalid_output_limits`. `ProcessOutcome::successful_exit()` is the narrow process-success predicate. | `spawn_failed`, `process_tree_containment_unavailable`, and `invalid_output_limits` are effect-not-started known failures. The other listed process failures are effect-started known failures unless trusted pre-child evidence proves otherwise. `ProcessOutcome::unavailable` means known pre-spawn failure only when the owner records that boundary; when it only means that the executor has no OS-process owner or the evidence is absent, it is incomplete evidence and any possible effect remains `unknown`. Process cleanup, timeout, kill/reap, and output redaction remain executor-owned. A process outcome is not by itself ProductTask terminalization. |
