...
- Branch: `main` includes trusted-local execution gate alignment merge `523db20`; this status was refreshed after post-merge main run `27967612746`.
- Tests: full Rust + TypeScript stack verification, 52 Python SDK tests, 154 HTTP tests, focused trusted-local/adaptive/provider-executor tests, rustfmt, Clippy, handoff, wire drift, dashboard boundary lint, and `git diff --check` passed with 0 failures on 2026-06-22. The supervised patch CLI repair test and Trial 4/5 CLI pilot stubs were hardened for noexec `/tmp` environments by keeping fake CLI executables under `target/`.
