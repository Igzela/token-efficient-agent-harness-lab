...
| Efficiency benchmark adapter | `engine/src/efficiency_benchmark_runtime.rs:invoke_langgraph`, reached only from benchmark strategy paths | Uses `uv` with a cleared environment and bounded output, but this helper currently uses `wait_with_output` without the shared timeout/process-tree cleanup contract. It is classified as benchmark-only evidence, not a managed ProductTask executor; no acceptance or authority may rely on it. | Explicit AC1 migration candidate: either keep it outside production execution and document that boundary, or migrate it under a compatible supervisor before any production use. Do not silently treat its current wait as equivalent to bounded supervision. |
| Target-repository Git output | `engine/src/target_repo_output.rs:run_git_inner` and its worktree/patch/publish callers | Uses bounded stdout/stderr readers, polls the child, kills and waits on timeout, and validates path/remote/branch/patch boundaries before output operations. Worktree removal and rollback remain target-output owners; target writes are separately authorized and were not invoked here. | Separate owner-preserving adapter group. Golden evidence is the target-repo output and recovery test family; AC1 must not move Git output or rollback authority into the scheduler. |
