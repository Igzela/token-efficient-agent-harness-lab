...
| Plan-lane successor promotion and escalation | `COMPLETE` | PR #387 accepted; exactly-one successor-promotion receipts and bounded pause escalation are controller-owned on the ledger |
| Route automation (continuous route driver) | `READY_FOR_EXECUTION` | Current promoted packet; automatic successor-contract compilation from `docs/FUTURE_ROUTE.md`, canonical closeout/promotion PR lifecycle, continuous route driver across packet boundaries |
...
8. Plan-lane admission, subject-aware terminal-owner readiness (PR #382), controller-owned plan-packet lifecycle receipts (PR #385), and exactly-one successor promotion with bounded pause escalation (PR #387) are accepted; the next window wires automatic successor-contract compilation from `docs/FUTURE_ROUTE.md`, the canonical closeout/promotion PR lifecycle, and a continuous route driver that repeats across packet boundaries without a planning agent manually authoring each successor.
9. The route driver must close and promote successors automatically (compiled, not manually authored) so a weak worker can cross multiple packet boundaries; successor execution, EFFECT execution, and automatic T3 handling remain explicitly inactive, and the blocked PREFLIGHT stays blocked until its B1/B2/provenance repair is accepted.
10. The `agent-controller.yml` workflow does not yet expose the `lifecycle-plan` and `promote-plan` controller commands that the plan worker dispatches, so those receipts are not yet producible in production; wiring them is a bounded planning-diff repair for the plan lane, not authority for any worker to edit `.github/workflows/`.
11. The `PE7-ROUTE-AUTOMATION-1` capsule `allowed_paths` omits `tests/test_agent_plan_lifecycle.py` although the packet's twelve-field contract item 9 names it among the focused tests to update; the capsule governs edits, so this packet reverts the out-of-capsule test change and covers the closeout-evidence-to-compiler binding in `tests/test_agent_plan_promotion.py` instead. The planning process should reconcile the contract/capsule list inconsistency (or drop the item-9 mention) when the next capsule is authored. `docs/MODULE_MAP.md` ownership rows for `scripts/agent-control/route_driver.py` likewise await a capsule-authorized documentation sync.
