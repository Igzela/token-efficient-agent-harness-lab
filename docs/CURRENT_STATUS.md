...
| Wire, codegen, SDK, Dashboard | `wire_contract/v1/` → `codegen/generate_wire_types.py` → Rust/Python/TypeScript generated outputs; Axum API handlers; Dashboard `createProductTask`, `compileAndScheduleProductTask`, `finalizeProductTask`, `approveProductTask`, `outputProductTask`; existing projection components | Generated types and Dashboard/SDK data are projections of Rust/API contracts, not authorities | No wire/schema migration in AC0; `scripts/check_wire_codegen_drift.sh` remains the drift gate; AC6 owns any later schema migration |
...
| Compile/execute/verify | `api_compile_and_schedule_product_task` → `compile_and_schedule_product_task` → `graph_ready` → `running` → `verifying`; then `awaiting_approval`/`output_pending`/`completed` or `failed`/`killed`/`paused`/`budget_exhausted`/`blocked`/`outcome_unknown` | `engine/tests/test_product_golden_path_g2.rs`, `test_product_golden_path_g3.rs`; PostgreSQL anchor `postgres_managed_executor_receives_exact_long_objective_without_public_persistence` is explicit and provider-free, while remaining steps are not claimed as per-backend equivalent here |
...
4. The AC0 data inventory is in progress as a provider-free contract packet; AC1 shared ProcessSupervisor hardening remains deferred and is not an active implementation frontier, while AC2–AC7 and the trace/order closeout remain gated by their immediately preceding accepted current-main contract packet.
