...
- IAE-3 extends the existing dashboard snapshot with effective provider/adaptive/default-routing/experiment/promotion/task-advancement authority; configured cost, traffic, token, call, time, concurrency, rollout, and worker bounds; safe observation aggregates; and secret-free scheduler state. Experiment and auto-promotion authority fail closed when their runtime policy or rollout configuration is invalid, and the dashboard exposes only safe validation blocker codes. The Adaptive Fusion page reuses existing authenticated scheduler pause/resume/kill and policy rollback endpoints and loads recent adaptive/scheduler actions through the existing `audit:read` API with `redact=true`. It never renders audit details.
...
- Branch: `main` at operator-validity hardening merge `0926f4e`.
...
- Browser: authenticated stub runtime showed effective authority, 3% experiment traffic, configured cost/worker ceilings, one safe observation, redacted adaptive/scheduler audit actions, and functional confirmed pause/resume controls. A separate invalid-policy runtime verified experiment/promotion authority fail closed, safe blocker rendering, no desktop or 390px mobile horizontal overflow, and no console errors.
...
- CI: operator-validity hardening PR #100 workflow run `27940515194` was green across all seven jobs and merged as `0926f4e`; the prior IAE-3 PR #99 and merge-to-main runs were also green.
