...
Accepted `main` contains the provider-free compatibility-calibration mechanism, the distinct versioned v2 refreeze, the repository-maintenance route implementation, and the accepted control-binding integrity repair. It contains **no accepted v2 preflight receipt** and **no v2 four-cell viability result**. PR #405's original invalid review records remain visible and non-authorizing; the additive retrospective correction proves only that the actual merged code later received exact-head `PASS`, with `historical_merge_compliant=false`. PR #411 accepted the fresh provider-free bootstrap authority, but accepted-main validation then confirmed a SQLite workspace-preparation receipt/status observation race before route start. `docs/NEXT_DECISION.md` therefore inserts one bounded repair while the route remains emergency-stopped. Neither the route controller nor a compiled successor may issue or consume external-effect authority, call a Provider, write a target, run an external schedule, or authorize downstream measurement/Architecture Convergence work outside each independently accepted packet.
...
| SQLite workspace-preparation receipt race repair | `READY_FOR_EXECUTION` | Sole current window; make SQLite receipt/status observation transactionally consistent without changing schema or recovery semantics |
...
8. Accepted-main validation exposed a SQLite workspace-preparation receipt/status TOCTOU: a duplicate intake can observe an earlier receipt miss and a later atomic `workspace_preparing` publication, then incorrectly classify the valid concurrent owner as a legacy missing-receipt state. The bounded repair is the sole current window.
9. Route bootstrap reconciliation and the first canonical provider-free soak have not yet run. PR #411 accepted one new attempt, but it remains blocked until the SQLite race repair is accepted; the terminated route10 material remains only non-resumable evidence from an obsolete main and cannot authorize or seed that attempt.
