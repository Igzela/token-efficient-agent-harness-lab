...
| V2 provider-free viability preflight | `DECISION_REQUIRED` | B1 timestamp and created_at provenance are on main; B2 freeze duration has no accepted owner |
...
1. No accepted provider-free v2 preflight receipt or viable four-cell v2 RWE baseline exists. Store-derived B2 authorization expiry has no accepted freeze duration on current main; caller-supplied `expires_at` remains and is not a completed repair.
