...
## AC0 Runtime Inventory

The current `PE7-AC0-RUNTIME-INVENTORY-1` work records a provider-free inventory without changing source behavior, authority, Provider access, target output, or the parked DB evidence. This section is candidate evidence only; packet lifecycle and route state remain owned by `docs/NEXT_DECISION.md`, and this is not an accepted packet receipt.

| Runtime family | Exact callers and owner | Spawn / timeout / cancellation / failure / rollback boundary | Bounded migration decision and golden evidence |
|---|---|---|---|
| Scheduler and generic executor | `engine/src/scheduler.rs:create_scheduler_executor`; `engine/src/executor_pool.rs:register_default_executors`; `engine/src/node_executor.rs:NodeExecutor`, `CommandNodeExecutor::execute_node`; `ToolPolicyNodeExecutor` remains the policy wrapper and `LocalProductStore` remains admission/receipt owner | Executor selection fails closed to `FailNodeExecutor`. Command execution revalidates argv/cwd/env, drains capped stdout/stderr, polls `try_wait`, and on timeout/read failure kills then waits; no fabricated exit code. Scheduler cancellation pauses store state and cancels the existing OpenCode handle. | Preserve the selection and policy boundary; any later supervisor must be an adapter, not a scheduler/store owner. Exact traces: `command_task_selects_policy_wrapped_command_never_noop`, `test_command_echo_ok`, `test_command_timeout_kills`, `test_command_output_is_truncated`, and `test_command_nonzero_exit`. |
| Managed CLI and runtime identity probes | `engine/src/cli/mod.rs:spawn_with_timeout_with_limits`; `CliNodeExecutor::execute_node` direct child path at `cli_node_executor.rs:221,371-412`, managed gateway path `execute_product_codex_with_budget_gateway:1027`, and profile probe `managed_coding_profile.rs:run_probe:367-376`; `config.rs:admit_claude_code_from_env`, `detect_binary:692-700`, `probe_claude_code_version`, and `probe_codex_version` are the remaining callers | The helper owns concurrent output caps, retry, timeout, reader failure, process-tree termination, reap, and reader join. `which` has no timeout, cancellation, kill, or explicit reap boundary and returns `None` on command failure. Managed Codex revalidates executable/profile and lease before the child; terminalization and rollback remain store-owned. | Candidate compatibility group is the helper plus the exact CLI/profile/config callers and the explicitly weaker `which` discovery call. Exact traces: `managed_cli_preserves_successful_process_exit`, `managed_cli_timeout_has_no_fabricated_exit_code`, `managed_cli_output_limit_fails_without_retaining_partial_output`, `managed_cli_spawn_failure_is_explicit`, `product_scheduler_executor_uses_store_lease_before_gateway_or_child_spawn`, `runtime_owner_preflight_blocks_before_managed_child_spawn`, `replacement_between_probe_and_spawn_fails_closed`, and `symlink_and_profile_mutation_fail_closed`. |
| Codex isolation/admission probes | `codex_mediation_admission.rs:unprivileged_user_ns_available:163-199` (direct `status()` probe; used by `build_mediated_codex_launch_plan`/`plan_mediated_codex_launch_for_gateway`), `probe_real_auth_hidden`; `codex_residual_admission.rs:probe_unshare_net_available`, `probe_loopback_only_unix_bridge_design`, `investigate_user_pid_namespace`; callers include `evaluate_residual_admission`, `codex_managed_acceptance_preflight.rs:290,309`, `codex_managed_acceptance_dry_run.rs:505,1060`, and `codex_partial_mediation_authority_decision.rs:432`, plus the real child spawn at `cli_node_executor.rs:1408` | The probes invoke `/usr/bin/bwrap` with direct `status`/`output` at `codex_mediation_admission.rs:168-199,782-840` and `codex_residual_admission.rs:239-269,363-400,607-634`; they have no shared wall-clock timeout or explicit kill/reap helper. Unsupported, non-success, and outcome-unknown capability evidence remains fail-closed and never becomes ProductTask success. | Keep probes as capability evidence; do not fold them into managed execution without preserving the evidence taxonomy. Exact fixtures: `capability_report_is_partial_not_full_when_bwrap_present`, `launch_plan_hides_real_credentials_and_uses_pid_isolation`, `launch_plan_rejects_non_loopback_gateway`, `sealed_helper_environment_drops_injected_parent_credentials`, `isolation_probe_hides_synthetic_auth_path`, `isolation_probe_hides_parent_environ_with_pid_namespace`, `residual_verdict_is_no_go_while_retry_identity_unproved`, `retry_identity_rejects_heuristic_accounting`, `network_bridge_design_probe_is_executed_when_unshare_net_available`, `user_pid_namespace_is_typed_never_silent_success_on_failure`, `full_admission_requires_all_axes_including_product_network_enforcement`, `default_input_is_blocked_identity_or_budget`, `lease_bound_partial_trial_requires_persisted_operator_acknowledgement`, and `manifest_never_contains_secret_shapes`. |
| OpenCode weak-worker adapter | `main.rs:346-351` → `OpenCodeNodeExecutor::execute_inner` → `engine/src/opencode_runtime.rs:1465-1467:OpenCodeProcessInvoker`, called by the existing scheduler/OpenCode path | Clears env except exact `PYTHONPATH`, creates a session, bounds both streams, observes cancellation/kill-switch/timeout, kills the process group, reaps the parent, joins readers, and records cleanup failure as failure/unknown. | Keep this specialized process-group owner until a later contract proves parity; do not create a second runtime. Exact traces: `kill_switch_fails_closed`, `cancellation_handle_stops_process_adapter`, `process_adapter_timeout_records_truthful_termination_evidence`, `stdin_write_failure_terminates_and_reaps_process_group`, `early_adapter_exit_with_descendant_is_contained`, `scheduler_stop_cancels_running_opencode`, and `supervised_workers_kill_switch_cancels_running_opencode`. |
| LangGraph external runtime | `main.rs:324-330` → `ExternalRuntimeNodeExecutor::execute_inner` → `engine/src/external_runtime.rs:736:LangGraphProcessInvoker`, gated by `ExternalRuntimeConfig` and live/provider controls | Clears env and bounds readers, but after spawn an unavailable stdin, stdin write failure, stdout/stderr extraction failure (`:215-231`), kill-switch, timeout, or wait failure returns a typed error; only the latter paths explicitly kill/wait. Live mode is forbidden in CI and no live call belongs to this inventory. | Keep separate because provider/effect gates and cost caps differ. Exact available fixture: `fixture_execution_is_idempotent_restart_safe_and_scope_bound`; no dedicated timeout/early-spawn cleanup fixture exists in this module, so those gaps remain recorded rather than invented as acceptance evidence. |
| Benchmark-only LangGraph adapter | `engine/src/efficiency_benchmark_runtime.rs:invoke_langgraph`, callers `langgraph_strategy:764`, `live_langgraph_strategy:809`, tool-adapter map `:1023`, and non-live strategy map `:1087` | Uses cleared env and `wait_with_output` at `:754-760`; stdout is collected before the 131072-byte check, so capture itself is not bounded. Stdin acquisition/write failure and `wait_with_output` failure after spawn return without explicit kill/reap. No timeout/process-tree cleanup exists. | Keep all four callers outside managed ProductTask execution, or migrate all four under a compatible supervisor in a later bounded packet; never treat current benchmark output as acceptance evidence. The only real adapter fixture is `langgraph_tool_bridge_invokes_the_real_adapter` (`:1368-1369`), marked `#[ignore]` and therefore not default-executable evidence. |
| Target-repository Git output | `engine/src/target_repo_output.rs:run_git_inner:1390`, via `run_git:1370`, `run_git_with_identity:1378`, `run_git_with_index:1387`; used by inspection, worktree, patch, publish, and rollback callers | Reads PATH/proxy/token/username/index/identity configuration; caps stdout/stderr and normally kills/waits on timeout. The `try_wait` error at `:1486` returns without kill/wait/reader join and is a known cleanup gap. Worktree removal, target writes, and rollback remain target-output owners and were not invoked. | Preserve the Git/rollback owner; repair or migrate the error path only under a later contract. Exact evidence: `read_only_patch_snapshot_preserves_real_git_index`, `truncated_worktree_registration_listing_fails_closed`, `github_create_payload_is_always_draft`, `outcome_unknown_reconciliation_cannot_authorize_another_pr_post`, `github_pr_config_rejects_noncanonical_api_origin`, and `github_response_must_be_open_draft_in_admitted_repository`. |
| Repository-maintenance controller (explicitly outside AC1 product-runtime scope) | Existing `scripts/agent-control` local supervisor, run-once, verification, route, worktree, ledger, and artifact owners; these scripts are repository-maintenance control plane, not Rust ProductTask runtime | Only the worker-child path owned by `local_run_once._bounded_process` is summarized here. Other helpers, including `prompt_builder.py:67`, `local_loop.py:448`, `control_state.py:57`, and `local_verification.py:259`, have independent `subprocess.run` boundaries and may not have process-group containment or sanitized inherited environment; this packet makes no claim about them. | Keep the existing repository-maintenance owner unchanged. Any future maintenance repair gets its own bounded packet; no `ProcessSupervisor` or product authority is introduced here. Exact worker traces remain `test_run_once_child_uses_isolated_session_not_caller_group`, `test_bounded_process_timeout_does_not_signal_caller_process_group`, and `test_terminate_task_process_group_kills_stubborn_descendants`. |
| Fixture/test-only process calls | Rust/Python tests invoke disposable `git`, `sh`, `sleep`, `python3`, and temporary worktree commands | Cleanup is test-local; these calls cannot authorize ProductTask, Provider, target, or acceptance state. A fixture failure is a test failure or skip, never production success. | Preserve as conformance evidence only; no AC1 production migration without a later explicit packet. |

The inventory classifies the current Rust production/runtime families and explicitly separates the repository-maintenance control plane without introducing `ProcessSupervisor`. It deliberately records weaker benchmark and probe paths as bounded future decisions rather than silently treating them as safe; no AC1 implementation or parked DB replay is authorized by this evidence.
