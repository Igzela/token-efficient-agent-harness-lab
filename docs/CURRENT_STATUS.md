...
The current `PE7-AC0-RUNTIME-INVENTORY-1` work records a provider-free inventory without changing source behavior, authority, Provider access, target output, or the parked DB evidence. The accepted route remains `READY_FOR_EXECUTION` until this evidence is independently accepted and its route transition is recorded; this section is candidate evidence, not an accepted packet receipt.
...
|---|---|---|---|
| Scheduler and generic executor | `engine/src/scheduler.rs:create_scheduler_executor`; `engine/src/executor_pool.rs:register_default_executors`; `engine/src/node_executor.rs:NodeExecutor`, `CommandNodeExecutor::execute_node`; `ToolPolicyNodeExecutor` remains the policy wrapper and `LocalProductStore` remains admission/receipt owner | Executor selection fails closed to `FailNodeExecutor`. Command execution revalidates argv/cwd/env, drains capped stdout/stderr, polls `try_wait`, and on timeout/read failure kills then waits; no fabricated exit code. Scheduler cancellation pauses store state and cancels the existing OpenCode handle. | Preserve the selection and policy boundary; any later supervisor must be an adapter, not a scheduler/store owner. Traces: `command_task_selects_policy_wrapped_command_never_noop` and node-executor timeout/read/spawn tests. |
| Managed CLI and runtime identity probes | `engine/src/cli/mod.rs:spawn_with_timeout_with_limits`; callers `cli_node_executor.rs:1408`, `config.rs:admit_claude_code_from_env`, `managed_coding_profile.rs`; `config.rs:detect_binary:692-700` directly runs `which` from Codex discovery; `probe_claude_code_version` and `probe_codex_version` use the bounded helper | The helper owns concurrent output caps, retry, timeout, reader failure, process-tree termination, reap, and reader join. `which` has no timeout, cancellation, kill, or explicit reap boundary and returns `None` on command failure. Managed Codex revalidates executable/profile and lease before `execute_product_codex_with_budget_gateway`; terminalization and rollback remain store-owned. | Candidate compatibility group is the helper plus its three caller families and the explicitly weaker `which` discovery call. Traces: `spawn_with_timeout_*`, config admission tests, profile mutation tests, `execute_product_codex_with_budget_gateway`, and CLI terminal-class tests. |
...
| Repository-maintenance controller (explicitly outside AC1 product-runtime scope) | Existing `scripts/agent-control` local supervisor, run-once, verification, route, worktree, ledger, and artifact owners; these scripts are repository-maintenance control plane, not Rust ProductTask runtime | The existing controller retains its own sanitized child environment, bounded process/tree cleanup, timeout reconciliation, and receipt ownership. This packet does not claim to inventory or migrate every maintenance helper; it records the boundary so those commands cannot be mistaken for a second product runtime or an AC1 caller. | Keep the existing repository-maintenance owner unchanged. Any future maintenance repair gets its own bounded packet; no `ProcessSupervisor` or product authority is introduced here. |
