...
| Route automation | `BLOCKED_PREREQUISITE` | PR #390 accepted the route implementation, but the first repaired-main bootstrap proved that GitHub rejects the sole `agent-controller.yml` dispatch surface because its 29 `workflow_dispatch` inputs exceed the platform limit of 25; route remains stopped pending the bounded input-limit repair |
| `PE7-CONTROLLER-DISPATCH-INPUT-LIMIT-REPAIR-1` | `READY_FOR_EXECUTION` | This planning/authority packet permits one single-owner, fail-closed transport repair that restores the existing controller workflow without executing T3 or any external effect |
...
8. A fresh route bootstrap from accepted main `aa83ac1f5eada74199e0ce28ecb91d37a48769d6` stopped with `route_controller_unavailable_timeout`. The exact canonical dispatch reproduced HTTP 422 because `.github/workflows/agent-controller.yml` defines 29 `workflow_dispatch` inputs while GitHub accepts at most 25. No workflow run, PR, claim, Provider call, target write, or external effect was produced. Route remains stopped pending `PE7-CONTROLLER-DISPATCH-INPUT-LIMIT-REPAIR-1`; route10 remains non-resumable obsolete-main evidence.
