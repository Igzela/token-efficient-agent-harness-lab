...
| `PE7-RWE-DB-RUN-1` | `COMPLETE` | Latest separately authorized run `run-goal-db-baseline-20260814-2340` / `auth-goal-db-run-20260814-2340`, same tenant `local`, executed all four frozen cells and ended `4/4 controlled_failure`; run evidence sha256 `a841e6d092d2946de2ee96bef03409ab8c276111c3ace53aef827bd0c00c277e`; durable usage was 8 requests, 19,274 tokens, cost `0.0051961592`; all four ProductTasks ended `failed/execution_failed`, all four workspaces were reconciled to `cleaned`; no terminal/reviewer evidence, Draft PR, or target-default-branch write exists, so disposition is `INSUFFICIENT`, not a viable baseline |
...
| Measurement readiness | `BLOCKED_PREREQUISITE` | Estimands, finite corpus/sampling, operations/evidence, protocol freeze, and reconstructable snapshot are accepted; the DB RUN is controlled-failure/`INSUFFICIENT`, so no decision-grade baseline exists |
| `PE7-RWE-DB-RUN-1` | `COMPLETE` | Four frozen cells completed under finite one-use authorization; all failed closed before review/verification, with durable usage and cleanup evidence recorded above |
| Decision-grade pre-AC baseline | `BLOCKED_PREREQUISITE` | Snapshot/corpus, preflight, and DB RUN are closed; only the DB analysis closeout remains |
