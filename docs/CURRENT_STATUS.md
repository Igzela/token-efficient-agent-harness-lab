...
- bounded replacement smoke Issue #231 created PR #232 (`docs/agent-smoke-replacement-20260717-c.md` only) and exact-head canonical seven-job CI passed (run `29565496618`), but the GitHub-hosted `agent-ci-monitor` workflow that dispatches independent review did not fire because `workflow_run` is not dispatched for `workflow_dispatch`-triggered bot CI. PR #233 repaired the dispatch-trigger path (squashed into `main` at `282cfc6246cdd0bdd607f70a951e5709e73f9ffa`); a follow-up smoke (Issue #234) was dispatched through `agent-controller` and reached the expected fail-closed terminal state on run `29572989439`. The original smoke PR remains unmerged and Issue #208 is emergency-stopped;
...
PR creation then failed because Actions PR creation was disabled at that time. PR #222 added setting preflight, bounded worker timeout, terminal phase/reason evidence, and capacity-safe cleanup. The repository setting, runner online/idle readiness, and empty-workspace repair are merged, the dispatch-trigger gap in `agent-ci-monitor` was repaired by PR #233, but the replacement smoke has not completed through independent exact-head review.
...
- complete one documentation-only replacement smoke through PR creation, exact-head CI, independent review, and manual merge decision before removing the restriction. The original smoke reached PR creation and exact-head CI but was blocked by the missing `workflow_dispatch` trigger in `agent-ci-monitor`; PR #233 repaired that gap and a follow-up smoke (Issue #234) reached the fail-closed gate while Issue #208 is still emergency-stopped;
