...
| Post-LGB Product Evolution Plan | PE-1, PE-2, and PE-3 are acceptance-sealed; PE4-CONTRACT-REPAIR-1, PE4-OFFLINE-1, PE4-READ-1, and PE4-SHADOW-1 are merged; PE4-CANARY-1 is active; promotion is prepared but not merged; PE4-CLOSE-1, PE-5, and PE-6 remain unstarted |
...
| PE-4 | P1/P2 | Trace-backed Policy Replay | Contract, offline replay, read surfaces, and shadow comparison are merged; PE4-CANARY-1 is active; promotion and closeout remain unmerged; PR #193 remains only a caller-asserted prototype and is not replay evidence |
...
- Focused replay tests cover observed/counterfactual separation, determinism, policy binding, source tampering, stale input, and OOD candidate selection. PE4-READ-1 adds additive schema v20 `offline_replay_artifacts` storage with SQLite/PostgreSQL parity, hash-bound idempotent recording, metadata-only audit, deterministic paginated/status-filtered read routes, OpenAPI parity, encoded Python/TypeScript readers, and explicit DynamicRegulator empty/insufficient-invalid-OOD/error states. It is merged; no live mutation, provider call, or promotion authority is added.

## PE-4 Shadow Evidence

- PE4-SHADOW-1 is merged as PR #200 (`54b3d46192f1de9b0bbaf1a1d83a7abaafff0201`, exact head `51a4b3a8b89ad1f65d74767b452e2546cf2526d7`). The hash-bound `ShadowReplayComparison` preserves observed facts separately from counterfactual predictions, binds source coverage and policy/candidate versions, records drift and insufficiency/OOD boundaries, and keeps every live-influence flag false. It cannot call providers or change routing/policy state.

## PE-4 Canary Evidence

- PE4-CANARY-1 adds a bounded `adaptive_canary.v1` decision through the existing experiment owner. It binds policy/candidate/scope versions and hashes to sufficient shadow evidence, requires default-off gates, explicit confirmation and permission, limits traffic to 1–5% and duration to 24 hours, and honors existing pause/kill gates.
- The decision carries a canonical content hash, idempotency key, compensation-required marker, and rollback target. Repeated evaluation is deterministic across restart and tampering fails validation; it is not a persisted experiment state machine and has no provider, routing, policy, budget, or full-rollout authority.
