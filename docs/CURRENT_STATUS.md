...
- Latest accepted governing-document baseline: PR #570 exact head `ec3cf3b90831de268a0a1b9f6afee5d093872c89` squash-merged as `37a4f752b2fdc3516b7581dffedcebb99c76f6a7`; its exact-head dual review and canonical CI accepted the provider-free preflight stop plus the bounded T2 read-only repair window. The predecessor remains `DECISION_REQUIRED`; the repair is not a live-ready claim and authorizes no Provider/effect path.
...
4. The AC0 data/trace freeze is complete as a provider-free contract and closeout; AC1 shared ProcessSupervisor hardening remains deferred and is not an active implementation frontier. AC2–AC7 are accepted through the closeout merge `42fcfa5a`; `PE7-RWE-CR-RECONSTRUCTION-1` is accepted through `7cfa817a`; `PE7-RWE-CR-PROTOCOL-PREFLIGHT-1` remains parked at `DECISION_REQUIRED`; and `PE7-RWE-CR-PROTOCOL-PREFLIGHT-REPAIR-1` is the current provider-free read-only implementation window. All replay effects remain gated successors.
...
9. The invalidated historical packets from `PE7-AC3-ORCHESTRATOR-CORE-1` through `PE7-HE-EC2-CONTRACT-1` (35 materially unfulfilled packets plus one separately invalid `PE7-HE-EC2-CONTRACT-1` receipt), along with earlier superseded route-automation receipts (`PE7-SUCCESSOR-PROMOTION-ESCALATION-1` and `PE7-ROUTE-AUTOMATION-1`), remain non-authorizing historical evidence. Accepted AC3–AC7 and Contemporary RWE reconstruction receipts are listed above; the predecessor semantic frontier remains parked at `PE7-RWE-CR-PROTOCOL-PREFLIGHT-1`, while `PE7-RWE-CR-PROTOCOL-PREFLIGHT-REPAIR-1` is the bounded implementation window awaiting completion and acceptance. Every downstream packet must be executed sequentially with genuine evidence, exact-head reviews, and canonical CI before promotion.
10. `PE7-RWE-CR-PROTOCOL-PREFLIGHT-1` remains `DECISION_REQUIRED` on accepted main `37a4f752b2fdc3516b7581dffedcebb99c76f6a7`. The provider-free preflight was not run: the default Store is absent, opening it can create/modify SQLite state and read the encryption-key symbol, authentication updates `last_used`, and `operator_preflight` reads a credential value. The current freeze is also an old single-arm binding rather than the required old/new identities, interleaving, concealment, drift, capacity, and finite-package proof. No database, credential, authority, Provider, target, or effect was touched. `PE7-RWE-CR-PROTOCOL-PREFLIGHT-REPAIR-1` is now the bounded read-only implementation window; it must not claim a live-ready baseline, and `PE7-RWE-CR-RUN-1` remains blocked.
