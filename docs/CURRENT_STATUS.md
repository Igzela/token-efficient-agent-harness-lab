...
8. A fresh route bootstrap from accepted main `aa83ac1f5eada74199e0ce28ecb91d37a48769d6` stopped with `route_controller_unavailable_timeout`. The exact canonical dispatch reproduced HTTP 422 because `.github/workflows/agent-controller.yml` defines 28 `workflow_dispatch` inputs while GitHub accepts at most 25. No workflow run, PR, claim, Provider call, target write, or external effect was produced. Route remains stopped pending `PE7-CONTROLLER-DISPATCH-INPUT-LIMIT-REPAIR-1`; route10 remains non-resumable obsolete-main evidence.
