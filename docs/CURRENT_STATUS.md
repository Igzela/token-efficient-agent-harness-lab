...
| Route automation | `IN_PROGRESS` | PR #416 accepted the control-plane stabilization implementation; PR #418 completed bootstrap reconciliation; PR #422 made weak-worker authentication/usage failures a terminal typed route stop. The current soak authorizes only its claim-bound local OpenCode worker, one real code-and-document packet, and subsequent packet advancement remain required |
...
| Route-autopilot adversarial soak | `READY_FOR_EXECUTION` | Current canonical packet: one exact-claim local OpenCode worker invocation is permitted; promotion still requires a live current-main candidate and proves clean and failure/recovery crossings without target write, EFFECT, or T3 action |
