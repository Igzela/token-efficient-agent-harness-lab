...
- Adaptive Fusion AF-7 exposes AF-6 in the dashboard through a completion test panel, optional routing metadata display, read-only gate/experiment/promotion/default-routing/kill status, rollback snapshot cues, and an endpoint config panel for `stub`, `openai_compatible`, and `anthropic` endpoint metadata. Endpoint config stores only symbolic credential environment names, rejects raw-secret-shaped values, and reports when local config needs runtime restart/reload before it can become executor input. It adds no routing-core behavior, provider authority, target-output authority, DB migration, or default-on live execution.
