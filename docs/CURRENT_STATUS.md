...
| Managed CLI and runtime identity probes | `engine/src/cli/mod.rs:spawn_with_timeout_with_limits`; callers `cli_node_executor.rs`, `config.rs:admit_claude_code_from_env`, `managed_coding_profile.rs`; `config.rs:detect_binary` at `:692-700` directly runs `which` from the Codex discovery path; `config.rs:probe_claude_code_version` and `probe_codex_version` use the bounded helper | The helper owns concurrent output caps, retry, timeout, reader failure, process-tree termination, reap, and reader join. `which` has no helper timeout and returns `None` on command failure. Managed Codex revalidates executable/profile and lease before spawn; terminalization and rollback remain store-owned. | Candidate compatibility group is the helper plus its three caller families and the explicitly weaker `which` discovery call. Traces: `spawn_with_timeout_*`, config admission tests, profile mutation tests, and CLI terminal-class tests. |
| Codex isolation/admission probes | `codex_mediation_admission.rs:unprivileged_user_ns_available` (`build_mediated_codex_launch_plan`/`plan_mediated_codex_launch_for_gateway`), `probe_real_auth_hidden`; `codex_residual_admission.rs:probe_unshare_net_available`, `probe_loopback_only_unix_bridge_design`, `investigate_user_pid_namespace`; residual callers include `evaluate_residual_admission`, `codex_managed_acceptance_preflight.rs` fixture/owner-derived paths, `codex_managed_acceptance_dry_run.rs:505,1060`, and `codex_partial_mediation_authority_decision.rs:432` | The probes invoke `/usr/bin/bwrap` with direct `status`/`output` at `codex_mediation_admission.rs:782-840` and `codex_residual_admission.rs:239-269,363-400,607-634`; they have no shared wall-clock timeout or explicit kill/reap helper. Unsupported, non-success, and outcome-unknown capability evidence remains fail-closed and never becomes ProductTask success. | Keep probes as capability evidence; do not fold them into managed execution without preserving the evidence taxonomy. Fixtures: `sealed_helper_environment_drops_injected_parent_credentials`, `launch_plan_hides_real_credentials_and_uses_pid_isolation`, `isolation_probe_hides_synthetic_auth_path`, `isolation_probe_hides_parent_environ_with_pid_namespace`, `network_bridge_design_probe_is_executed_when_unshare_net_available`, `user_pid_namespace_is_typed_never_silent_success_on_failure`, `default_input_is_blocked_identity_or_budget`, and `lease_bound_partial_trial_requires_persisted_operator_acknowledgement`. |
...
| Repository-maintenance controller | `scripts/agent-control/local_supervisor.py:354` launches `local_run_once.py`; shared `_bounded_process` is `local_run_once.py:336-385`. Exact additional subprocess callers are `local_run_once.py:2269,2333,2466`, `local_verification.py:259,267,306`, `route_driver.py:1456,1467`, `state_manager.py:335`, `runner_readiness.py:38`, and `artifact_contract.py:127`. | Worker children use a sanitized env, new session, bounded communicate, TERM/KILL descendant cleanup, bounded reap, and truthful unknown reconciliation. Maintenance Git/GitHub/readiness/artifact helpers use 20/30/60/120-second `subprocess.run` timeouts and fail closed on timeout/nonzero; they are not ProductTask runtime or AC1 supervisor callers. | Keep the existing repository-maintenance, worktree, PR, ledger, and receipt owners; do not migrate these helpers as product runtime. Evidence: `test_run_once_child_uses_isolated_session_not_caller_group`, `test_bounded_process_timeout_does_not_signal_caller_process_group`, `test_terminate_task_process_group_kills_stubborn_descendants`, route-driver promotion tests, local-loop state tests, and artifact-contract scope tests. |
...
The inventory classifies the current production/runtime families and the repository-maintenance subprocess boundary without introducing `ProcessSupervisor`. It deliberately records weaker benchmark and probe paths as bounded future decisions rather than silently treating them as safe. The next permitted action is an independently reviewed route closeout/promotion; no AC1 implementation or parked DB replay is authorized by this evidence.
