...
| Measurement readiness | `BLOCKED_PREREQUISITE` | Estimands, finite corpus/sampling, and operations/evidence are accepted; protocol freeze is the current contract window and decision-grade baseline remains blocked |
...
3. No accepted operations/evidence manifest, larger decision baseline, or reconstructable contemporary old/new comparison exists; the finite corpus and estimand contracts are accepted but do not provide those capabilities.
