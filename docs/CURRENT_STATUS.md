...
| Harness-Evolution C0–C4 capability spiral | `BLOCKED_PREREQUISITE` | The accepted EC3 contract, instrumentation, and enforcement now precede current `PE7-HE-CL0-PILOT-1`; the 28 formal future packets progress from one usable verified-delivery loop through the Harness x Model x Strategy laboratory, one generation, bounded recursion, final sealed transfer, readiness, and human adoption decision; the pilot remains separately authorization-gated and no live effect is authorized |
...
5. Accepted Harness-Evolution identity/lineage, causal-manifest, mutation-registry, evaluator/holdout, holdout-seal, sentinel-conformance, and evaluator-owned `PredictionOutcomeV1` boundaries are provider-free and non-authoritative. The accepted EC3 contract freezes lifecycle-cost ontology, trust, missingness, envelopes, and accounting rules, but no lifecycle-cost instrumentation or enforcement is accepted. No accepted diversity/exploration or Pareto/stop/recovery implementation exists.
