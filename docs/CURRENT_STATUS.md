...
| `PE7-AC0-RUNTIME-INVENTORY-1` | `COMPLETE` | Accepted-main source bind `830b233dfc82a976e3205a7964efe6c76e42d6ae`; provider-free inventory below; no source behavior, Provider, authority, target, or store mutation |
...
## Accepted AC0 Runtime Inventory

`PE7-AC0-RUNTIME-INVENTORY-1` is complete as a provider-free architecture inventory. The inventory searched the accepted Rust production tree and the repository-maintenance process callers, then bound each executable family to its current owner, cleanup behavior, failure class, rollback point, and existing trace/fixture coverage. It introduces no `ProcessSupervisor`, changes no runtime behavior, and does not make the parked DB run relevant again.

| Runtime family | Current caller and owner | Spawn / timeout / cancellation / failure boundary | AC1 candidate group and golden evidence |
|---|---|---|---|
| Scheduler and executor selection | `engine/src/scheduler.rs:create_scheduler_executor`; `engine/src/executor_pool.rs:register_default_executors`; `engine/src/node_executor.rs:NodeExecutor`; policy/admission remains `LocalProductStore`-owned | Selection is typed and fail-closed: unavailable executors become `FailNodeExecutor`; scheduler kill pauses recursive execution in the store and cancels the existing OpenCode handle. This layer owns orchestration, not child-process cleanup. | Keep as the caller boundary. `executor_pool.rs:command_task_selects_policy_wrapped_command_never_noop` is the selection trace; no new scheduler owner. |
| Generic command executor | `engine/src/node_executor.rs:CommandNodeExecutor::execute_node`, wrapped by `engine/src/tool_policy_executor.rs:ToolPolicyNodeExecutor` | Revalidates allowlisted argv, workspace cwd, and environment; spawns `std::process::Command`; drains capped stdout/stderr; polls `try_wait`; timeout or read failure calls `kill` then `wait`; returns typed `ProcessOutcome` and never fabricates an exit code. Store policy is checked before the child and remains the rollback/receipt owner. | Candidate for the AC1 supervisor adapter, preserving tool-policy-before-spawn. Golden traces: `engine/src/node_executor.rs` command timeout/read/spawn tests and the executor-pool no-noop test. |
| Managed CLI, Codex, and runtime probes | `engine/src/cli/mod.rs:spawn_with_timeout_with_limits`; callers in `cli_node_executor.rs`, `config.rs:admit_claude_code_from_env`, and `managed_coding_profile.rs`; `config.rs:detect_binary` also directly invokes `which` for the Codex discovery path | The managed CLI helper owns process containment, concurrent output caps, transient spawn retry, timeout, reader failure, process-tree termination, parent reap, and reader join. `detect_binary` uses `which` with `output()` and no helper timeout. Managed Codex revalidates the exact executable/profile and store lease before spawn; terminalization stays with `LocalProductStore`. | Candidate compatibility group is the helper plus its three production caller families and the bounded `which` discovery call. Golden traces: `cli/mod.rs::spawn_with_timeout_*`, `config.rs` admission tests, `managed_coding_profile.rs` profile mutation tests, and `cli_node_executor.rs` pre-child/terminal-class tests. |
| Codex isolation/admission probe subprocesses | `codex_mediation_admission.rs:unprivileged_user_ns_available` is called by `build_mediated_codex_launch_plan` and `plan_mediated_codex_launch_for_gateway`; `probe_real_auth_hidden` is the provider-free isolation probe. `codex_residual_admission.rs:probe_unshare_net_available`, `probe_loopback_only_unix_bridge_design`, and `investigate_user_pid_namespace` are called by `evaluate_residual_admission` and `codex_managed_acceptance_preflight.rs:run_managed_acceptance_preflight`/owner-derived preflight paths | These probes directly invoke `/usr/bin/bwrap` with `status`/`output`; `--die-with-parent` and namespace isolation bound the child, but the probes have no shared wall-clock timeout or explicit kill/reap helper. Non-success, unavailable, and outcome-unknown classifications are preserved fail-closed; they are capability evidence, not ProductTask execution. | Fixed AC1 candidate group: the five named probe functions and the exact launch-plan, residual-admission, fixture-preflight, and owner-derived-preflight callers. Golden fixtures: `sealed_helper_environment_drops_injected_parent_credentials`, `launch_plan_hides_real_credentials_and_uses_pid_isolation`, `isolation_probe_hides_synthetic_auth_path`, `isolation_probe_hides_parent_environ_with_pid_namespace`, `network_bridge_design_probe_is_executed_when_unshare_net_available`, `user_pid_namespace_is_typed_never_silent_success_on_failure`, `default_input_is_blocked_identity_or_budget`, and `lease_bound_partial_trial_requires_persisted_operator_acknowledgement`. |
| OpenCode weak-worker adapter | `engine/src/opencode_runtime.rs:OpenCodeProcessInvoker`, called by the existing scheduler/OpenCode runtime; cancellation is `OpenCodeCancellationHandle` plus scheduler kill | Starts the adapter with a cleared minimal environment and exact `PYTHONPATH`, creates a Unix session, bounds both streams, observes cancellation/kill-switch/timeout, terminates the process group, reaps the parent, joins readers, and records containment evidence. Failed cleanup remains failure/unknown evidence; it is not converted to success. | Keep this specialized process-group path until AC1 proves semantic parity; do not merge it with generic command policy casually. Golden traces cover descendant spawn, cancel, timeout, kill-switch, early exit, and reader/containment failure. |
| LangGraph external runtime | `engine/src/external_runtime.rs:LangGraphProcessInvoker`; validated by `ExternalRuntimeConfig` and explicitly gated for auth/provider/live mode | Clears environment, passes only adapter inputs, bounds output/error readers, and kills/reaps on kill-switch, timeout, or wait failure. Live mode is forbidden in CI and requires its existing auth/provider/confirmation gates. | Separate adapter group because it has provider/effect gates and cost caps. The fixture path is covered by `external_runtime.rs`; no live call is part of AC0. |
| Efficiency benchmark adapter | `engine/src/efficiency_benchmark_runtime.rs:invoke_langgraph`, with fixed callers at `langgraph_strategy`, `live_langgraph_strategy`, the live tool-adapter map, and the non-live strategy map (`:764`, `:809`, `:1023`, `:1087`) | Uses `uv` with a cleared environment and bounded output, but this helper currently uses `wait_with_output` without the shared timeout/process-tree cleanup contract. It is classified as benchmark-only evidence, not a managed ProductTask executor; no acceptance or authority may rely on it. | Fixed AC1 decision: keep these four benchmark caller paths outside production execution, or migrate all four under a compatible supervisor before any production use. Do not silently treat the current wait as bounded supervision. |
| Target-repository Git output | `engine/src/target_repo_output.rs:run_git_inner`, reached through the remote inspection, worktree preparation/inspection/removal, patch staging/inspection/export, branch publish, and rollback callers (`:258`, `:447-1087`, `:1199-1245`) | Uses `PATH`, proxy, token, username, index, and identity configuration reads; bounds stdout/stderr and normally kills/waits on timeout. Important exception: `try_wait` error at `:1486` returns immediately without kill/wait or reader join, so that path is an explicit cleanup gap, not a success. Worktree removal and rollback remain target-output owners; target writes are separately authorized and were not invoked here. | Fixed owner-preserving AC1 candidate group is `run_git_inner` plus the listed Git callers; its timeout/wait-error behavior must be preserved or repaired under a later contract. Golden evidence is `test_target_repo_output.rs` and `test_product_golden_path_recovery.rs`; AC1 must not move Git output or rollback authority into the scheduler. |
| Repository-maintenance local loop | `scripts/agent-control/local_supervisor.py` launches `local_run_once.py`; `local_run_once.py` owns descendant discovery, TERM/KILL, child environment allowlist, and bounded receipt parsing | Supervisor enforces bounded active count and task timeout; timeout terminates the child tree and reconciles the attempt, otherwise returns `outcome_unknown`/`failed_unknown_output` when terminal state is unproven. Child env drops GitHub/provider/cloud credentials. | Keep under the existing repository-maintenance controller and worktree/PR owners. Golden evidence is `tests/test_agent_local_loop.py`, `test_agent_control_state.py`, `test_agent_orchestrator_*`, and local-verification tests; it is not a product runtime. |
| Fixture/test-only process calls | Rust and Python test fixtures use `git`, `sh`, `sleep`, `python3`, and temporary worktrees across `engine/tests`, `tests`, and `tools` | These calls exercise the production boundaries or disposable drills; fixture cleanup is local to each test and is not an authority or acceptance receipt. | Preserve as conformance coverage. They are not AC1 production callers unless a future packet explicitly proves a production path. |

### AC0 inventory conclusions

- The current runtime has one scheduler/executor owner and one persistence/authority owner; there is no second runtime or store to preserve.
- Direct production subprocess families are fully classified above. The only intentionally weaker path is the benchmark-only LangGraph helper, which is recorded as a bounded migration decision rather than hidden.
- AC1 may introduce an additive process-supervision seam only after its own contract packet freezes the API and preserves policy/admission, provider, scheduler, target-output, credential, and rollback ownership.
- The next packet is `PE7-AC0-DATA-CONTRACT-INVENTORY-1`; AC0 as a whole remains incomplete until data/contracts and trace/order freeze are accepted.
...
4. AC0 runtime inventory is complete and accepted as provider-free evidence; AC0 data/contract inventory and trace/order freeze remain unaccepted, and each implementation slice remains blocked until its immediately preceding current-main contract packet is accepted.
