...
Accepted `main` contains the provider-free compatibility-calibration mechanism, the distinct versioned v2 refreeze, the repository-maintenance route implementation, the accepted control-binding integrity repair, and the accepted SQLite workspace-preparation receipt-race repair. It contains **no accepted v2 preflight receipt** and **no v2 four-cell viability result**. PR #405's original invalid review records remain visible and non-authorizing; the additive retrospective correction proves only that the actual merged code later received exact-head `PASS`, with `historical_merge_compliant=false`. The fresh human planning decision is bound to accepted baseline `72c196aa03a5632bfbd47ba5f19cbc51a154889c` and authorizes the executable provider-free bootstrap window in `docs/NEXT_DECISION.md`; execution must still derive the live post-merge accepted main and has not yet produced new route evidence. Neither the route controller nor a compiled successor may issue or consume external-effect authority, call a Provider, write a target, run an external schedule, or authorize downstream measurement/Architecture Convergence work outside each independently accepted packet.
...
| Route automation | `BLOCKED_PREREQUISITE` | PR #390 accepted the route implementation, but the first repaired-main bootstrap exposed an invalid controller dispatch surface before any weak-agent packet could run; route remains stopped pending bounded end-to-end autonomy stabilization |
| `PE7-ROUTE-AUTONOMY-STABILIZATION-1` | `READY_FOR_EXECUTION` | This planning/authority packet permits one long-lived, single-owner, provider-free control-plane repair branch to resolve ordinary route/controller/worker defects until the canonical soak and one real implementation packet complete end to end |
...
| Route-autopilot adversarial soak | `BLOCKED_PREREQUISITE` | Next canonical provider-free successor after the newly authorized bootstrap; promotion still requires a live current-main candidate and proves clean and failure/recovery crossings without executing a Provider, target, EFFECT, or T3 action |
...
8. A fresh route bootstrap from accepted main `aa83ac1f5eada74199e0ce28ecb91d37a48769d6` stopped with `route_controller_unavailable_timeout`. The first exact defect is HTTP 422 because `.github/workflows/agent-controller.yml` defines 28 `workflow_dispatch` inputs while GitHub accepts at most 25. No workflow run, PR, claim, Provider call, target write, or external effect was produced. Route remains stopped pending `PE7-ROUTE-AUTONOMY-STABILIZATION-1`; that packet may repair further ordinary failures found while proving the same bounded provider-free lifecycle, without opening a new infrastructure repair PR for each defect. Route10 remains non-resumable obsolete-main evidence.
