...
| Managed CLI, Codex, and admission probes | `engine/src/cli/mod.rs:spawn_with_timeout_with_limits`; callers in `cli_node_executor.rs`, `config.rs`, and `managed_coding_profile.rs`; sandbox/admission probes in `codex_mediation_admission.rs` and `codex_residual_admission.rs` | One bounded helper owns process containment, concurrent output caps, transient spawn retry, timeout, reader failure, process-tree termination, parent reap, and reader join. Managed Codex revalidates the exact executable/profile and store lease before spawn; terminalization stays with `LocalProductStore`. | Candidate for a compatibility adapter, not a new authority. Golden traces cover timeout, descendant cleanup, output limits, missing binary, profile mutation, pre-child admission, and truthful terminal classes in the named CLI modules. |
| Codex isolation/admission probe subprocesses | `engine/src/cli/codex_mediation_admission.rs:unprivileged_user_ns_available` and `probe_real_auth_hidden`; `engine/src/cli/codex_residual_admission.rs:probe_unshare_net_available`, `probe_loopback_only_unix_bridge_design`, and `investigate_user_pid_namespace`; called by `codex_managed_acceptance_preflight.rs` and dry-run/admission decision paths | These probes directly invoke `/usr/bin/bwrap` with `status`/`output`; `--die-with-parent` and namespace isolation bound the child, but the probes have no shared wall-clock timeout or explicit kill/reap helper. Non-success, unavailable, and outcome-unknown classifications are preserved fail-closed; they are capability evidence, not ProductTask execution. | Fixed AC1 candidate group: all five named probe functions plus their preflight/dry-run callers. Golden fixtures/tests are the `#[test]` blocks in `codex_mediation_admission.rs`, `codex_residual_admission.rs`, and `codex_managed_acceptance_preflight.rs`; do not fold these probes into the managed CLI executor without preserving their evidence taxonomy. |
