...
| Measurement readiness | `BLOCKED_PREREQUISITE` | Estimands, finite corpus/sampling, operations/evidence, protocol freeze, and reconstructable snapshot are accepted; the DB RUN evidence exists but its route T3/owner-outcome closeout is not accepted |
| Decision-grade pre-AC baseline | `BLOCKED_PREREQUISITE` | Snapshot/corpus, preflight, DB RUN route closeout, and analysis; the DB RUN closeout is currently `DECISION_REQUIRED` |
