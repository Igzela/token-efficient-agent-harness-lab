...
| Provider-free RWE DB preflight | `BLOCKED_PREREQUISITE` | Snapshot verifier passed and the existing owner preflight test passed; real `rwe-live-baseline preflight` stopped before `operator_preflight` because no store-owned operator API-key/tenant binding or same-tenant completed Golden Path evidence was available; no zero-mismatch receipt, Provider call, authority consumption, or target write |
| Measurement readiness | `BLOCKED_PREREQUISITE` | Estimands, finite corpus/sampling, operations/evidence, protocol freeze, and reconstructable snapshot are accepted; real provider-free preflight remains unavailable |
...
3. No accepted operations/evidence manifest, larger decision baseline, or executed contemporary old/new comparison exists; the reconstructable snapshot verifier and owner preflight test pass, but the real CLI preflight cannot reach `operator_preflight` without a store-owned operator principal and same-tenant completed Golden Path evidence. No zero-mismatch receipt is claimed; fixture/dry-run stores are not managed acceptance.
