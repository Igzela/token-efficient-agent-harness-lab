...
| Known outcome | `known_success` requires `ProcessOutcome::exited(0)` plus the existing verification/terminal owner; `known_failure` includes non-zero `exited`, `signaled`, `output_read_failed`, and store-owned `Failed`, `BudgetExhausted`, or `Blocked` | `ProcessOutcome` describes the executor/process observation. `signaled` with an unavailable signal remains a known termination with incomplete detail; `unavailable` is incomplete process evidence, never success. `ProductTaskStatus::Completed` is the only canonical product success. |
...
| Adaptive provider adapter | `AdaptiveExecutionResult`/`AdaptiveExecutionError` remain adapter-local evidence; the product boundary must map success, bounded failure, kill/timeout, and possible-send uncertainty into the same effect/outcome vocabulary | Adaptive execution does not become a second scheduler, budget ledger, usage owner, or ProductTask authority. |
