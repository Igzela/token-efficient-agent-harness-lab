...
- Adaptive Fusion AF-7 exposes AF-6 in the dashboard through a completion test panel, optional routing metadata display, read-only gate/experiment/promotion/default-routing/kill status, rollback snapshot cues, and an endpoint config panel for `stub`, `openai_compatible`, and `anthropic` endpoint metadata. Endpoint config stores only symbolic credential environment names, rejects raw-secret-shaped values, requires present credential environment variables for real providers, and applies validated local config to the adaptive completion API without restart while leaving the startup-bound workflow/scheduler executor path unchanged. It adds no target-output authority, DB migration, or default-on live execution.
...
- Branch: `main` includes trusted-local execution gate alignment merge `523db20`; this status was refreshed after main CI run `27968114851`.
- Tests: full Rust + TypeScript stack verification, 52 Python SDK tests, 154 HTTP tests, focused trusted-local/adaptive/provider-executor tests, rustfmt, Clippy, handoff, wire drift, dashboard boundary lint, and `git diff --check` passed with 0 failures; latest full main CI evidence is run `27968114851`. The supervised patch CLI repair test and Trial 4/5 CLI pilot stubs were hardened for noexec `/tmp` environments by keeping fake CLI executables under `target/`.
...
- CI: trusted-local execution gate PR #106 run `27966637107`, post-merge main run `27967126428`, status-refresh main run `27967612746`, and final head-bound wording main run `27968114851` were green across all seven jobs. CLI pilot stub PR #105 run `27964232053`, noexec repair status main run `27964004871`, supervised repair CLI test main run `27963948784`, supervised repair CLI test PR #104 run `27963458311`, and trusted-local docs main run `27963122689` were also green.
