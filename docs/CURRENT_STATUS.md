...
- PE-5's prior acceptance remains under repair pending required CI. The repair implements separate exact-local SLSA, SPDX, and `release_provenance.v2` manifest bundles; immutable attested bootstrap assets; real Cargo/Bun dependency inventory; explicit first/previous-release state; bounded archive validation; and verified transactional install/rollback outcomes. Fresh-install success additionally requires a bounded execution of the installed binary's `--help`; any explicit operator health command runs only after that default gate, and either failure restores the pre-install Dashboard, preserves operator data, and removes partial activation. The grouped `PE5-CONTRACT-1` through `PE5-PUBLISH-1` milestones and PRs #210-#211 remain historical only where their semantics are weaker.
...
- Historical v1 release fixtures remain readable for compatibility tests but are structurally limited to `verified_fixture`, never production `verified`. This is enforced at the tool boundary: legacy `verify` and legacy attestation generation accept fixture mode/identity only, direct non-fixture legacy verification is `unsupported` and nonzero through the CLI, and only `verify-release` v2 can authorize production release evidence. No external production identity, real tag, public release, or production installation was exercised by this repair.
