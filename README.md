...
| **Claude Code as managed runtime** | **Default off / exact admission only** — pinned binary/version/hash, exact-pinned or CLI-resolved model with proven resolved identity, safe mode, at most three tool turns, bounded budget, app-owned worktree/path binding, and no Bash/network/subagent tools |
...
Provider API execution requires explicit endpoint/auth/budget configuration; CI uses stub/mock paths and does not call real provider APIs. A ready trusted-local profile activates bounded provider execution, adaptive routing, experiments, promotion, default routing, and acknowledged task advancement for internal local operation. Managed CLI execution is default-off. Codex retains its existing `workspace-write` adapter. Claude Code registration additionally requires an independent gate plus exact regular-file binary path, version, SHA-256, three-turn limit, and positive worst-case client-estimate cap; the model is either the exact admitted snapshot passed with `--model` or the CLI's own configured default (for example an operator subscription import) whose resolved identity must be proven from owner-reported usage evidence. Its invocation is safe-mode/no-session, strict-empty-MCP, `dontAsk`, Read/Edit/Write-only, worktree-bounded for reads, path-scoped for edits/writes, and denies Bash/network/subagent tools. The local dashboard remains guarded; dangerous actions require confirmation and audit logging.
...
- Managed CLI execution is default-off and requires `ACP_ENABLE_CLI_EXECUTION=1`, authenticated operation, an exact app-owned workspace binding, policy approval/receipt enforcement, and an inactive `ACP_CLI_EXECUTION_KILL_SWITCH`. Codex uses the existing `workspace-write` adapter. Claude Code additionally requires `ACP_ENABLE_CLAUDE_CODE_EXECUTION=1` and exact `ACP_CLAUDE_CODE_BIN`, `ACP_CLAUDE_CODE_VERSION`, `ACP_CLAUDE_CODE_SHA256`, `ACP_CLAUDE_MAX_TURNS=3`, and `ACP_CLAUDE_MAX_BUDGET_USD=2.16`; `ACP_CLAUDE_MODEL` is optional and, when set, must equal the admitted model snapshot. When it is unset, the invocation passes no `--model` and the admitted CLI resolves its own configured default model (for example an operator subscription import); the CLI must then prove exactly one resolved model identity in its owner-reported per-model usage, that identity is recorded as `resolved_model` in the node result and terminal usage evidence, and missing or ambiguous identity fails closed. First-party `ANTHROPIC_BASE_URL`/`ANTHROPIC_AUTH_TOKEN`/`ANTHROPIC_MODEL` variables pass to the child only when the operator explicitly lists them in `ACP_CLI_ENV_ALLOWLIST`; incomplete or mismatched admission stays unavailable.
