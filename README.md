...
- Batch 6 sandbox/workspace/approval/rollback/artifact-capture contracts and Batch 7 readiness audit are planning docs only, not runtime capability.
...
Keep the repo moving through the autonomous maintainer loop: repair CI/docs/test drift, maintain wire governance, keep docs current, and fix focused regressions. The R-series is sealed at R7. R8 is not approved. No further file splitting is approved. Supervised autonomous beta Batch 7 is NO-GO for implementation until a separate accepted implementation plan closes the readiness gaps in ADR-0002. Any work that adds cloud hosting, broadens model provider integration, adds sandbox isolation, expands subprocess execution beyond the existing CLI executor path, mutates target repos, adds hosted deployment, wires approval/run/deploy/merge controls, or adds real autonomous workers still requires explicit approval.
