...
| **Claude Code as managed runtime** | **Default off / exact admission only** — pinned binary/version/hash/model, safe mode, at most three tool turns, bounded budget, app-owned worktree/path binding, and no Bash/network/subagent tools |
...
Provider API execution requires explicit endpoint/auth/budget configuration; CI uses stub/mock paths and does not call real provider APIs. A ready trusted-local profile activates bounded provider execution, adaptive routing, experiments, promotion, default routing, and acknowledged task advancement for internal local operation. Managed CLI execution is default-off. Codex retains its existing `workspace-write` adapter. Claude Code registration additionally requires an independent gate plus exact regular-file binary path, version, SHA-256, admitted model snapshot, three-turn limit, and positive worst-case client-estimate cap. Its invocation is safe-mode/no-session, strict-empty-MCP, `dontAsk`, Read/Edit/Write-only, worktree-bounded for reads, path-scoped for edits/writes, and denies Bash/network/subagent tools. The local dashboard remains guarded; dangerous actions require confirmation and audit logging.
