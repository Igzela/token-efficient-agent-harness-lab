...
- Supervised autonomous beta planning is accepted as a planning-only track in `docs/adr/0002-supervised-planning-track.md`. Batch 0-6 governance/module/model/read-only-planner/durable-state/advisory/design-gate work is recorded, with `WorkflowGraph` selected as canonical planning model, `/api/v1/plans` storing non-executable app-owned SQLite plans plus recommendation-only quality/routing/retry/observability advisory metadata, `/api/v1/workflow-runs` storing inert workflow run/node/edge/event/approval metadata, and ADR/security docs recording future sandbox/workspace/approval/rollback/artifact-capture contracts. Batch 7 Slice A stores app-owned supervised patch workspace/artifact metadata in SQLite with path-boundary and import-validation tests outside registered target repositories. Batch 7 Slice B exposes that metadata through read-only HTTP GET routes only. Batch 7 Slice C exposes those GET routes through TypeScript/Python SDK read-only methods only. Batch 7 Slice D defines the docs-only approval-binding contract for future patch-review/export. These records, docs, and read-only views are not runtime autonomous workers and do not grant execution authority.
- Current Rust test count: 1205 pass.
- Dynamic Workflow: ALL 7 BATCHES COMPLETE. Minimum acceptance target achieved.
...
Batch 6 supervised-execution contracts, Batch 7 Slice A storage metadata, Batch 7 Slice B read-only HTTP metadata views, Batch 7 Slice C read-only SDK wrappers, and Batch 7 Slice D approval-binding design do not authorize runtime execution. Implementing sandbox/process/container/VM behavior, workspace directory creation, target workspace writes, patch generation, approval broker wiring, rollback execution, artifact-capture runtime, export runtime, or Batch 7 supervised execution still requires a separate explicit human-approved batch.
