...
The Rust `engine/` is the sole runtime, API, and storage implementation. The dispatch kernel, V2 output track, Adaptive Fusion through AF-7, Agent Runtime through AR-6, Trusted Local Autonomous Execution through IAE-3, the importer-first benchmark path, PE-1, PE-2, PE-3, PE-4, PE-5, and PE-6 are complete and acceptance-sealed. Later work remains governed by `docs/NEXT_DECISION.md`.
