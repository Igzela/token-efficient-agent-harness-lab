...
- Local small-team productization is implemented: app-owned SQLite dispatch history/config/team/API-key metadata/audit/cost/plan state, live dashboard API state, optional local API key role boundary, export, confirmed local backup, operations metrics, backup verify/restore dry-run, audit redaction, provider pricing visibility, local ops/restore smoke scripts, and SDK methods. Broader provider execution beyond the explicit env-gated local beta path, target writes, sandbox/process/container/VM isolation, runtime workers, cloud SaaS, and hosted production deployment remain disallowed.
...
- Supervised autonomous beta planning is accepted as a planning-only track in `docs/adr/0002-supervised-planning-track.md`. Batch 0-3 governance/module/model/read-only-planner work is recorded, with `WorkflowGraph` selected as canonical planning model and `/api/v1/plans` storing non-executable app-owned SQLite plans. A read-only planner is not a runtime autonomous worker when it only creates non-executable app-owned plans.
- Current Rust test count: 1184 pass.
