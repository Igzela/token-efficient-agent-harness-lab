...
- Do not use Vader or Issue #208 as product runtime; Issue #254 is parked. OpenCode may serve only as the explicitly packet-authorized repository-maintenance bounded autonomous worker through the existing controller and worktree boundary. It must not become a product runtime or create a second runtime, scheduler, store, evaluator, workspace, output, audit, or rollback owner.
