...
- Post-LGB Product Evolution — PE-1 and PE-2 are complete; PE-3 is active with its decision contracts, mutation-free derived queue, read-only surface, and hash-bound adapters to existing action owners implemented; later stages are governed by `docs/NEXT_DECISION.md`
