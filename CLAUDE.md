...
- **2026-06-06**: GA-7 Soak Test + Dormant Module Adaptation Track — Full dormant module audit: 76 files / 23,939 lines across 13 top-level modules + 21 sub-modules. 4-phase strategy (Interface Unification → Zero-Conflict Activation → Adapted Activation → Dead Code Cleanup) documented in NEXT_DECISION.md. `scripts/auto_adapt_loop.sh` created for autonomous phase advancement. 1286 Rust tests pass.
- **2026-06-06**: GA-7 Soak Test + Dormant Module Audit — `scripts/soak_ga_e2e.py` with 10 integration tests. Full dormant module audit: 76 files / 23,939 lines identified across 13 top-level dormant modules + 21 dormant sub-modules. 4-phase adaptation strategy documented in NEXT_DECISION.md. `scripts/auto_adapt_loop.sh` created for autonomous phase advancement. 1286 Rust tests pass.
...
- **2026-06-06**: Dormant Adaptation Phase 1 — Interface Unification. (1) `trait Evaluator` extracted from `EvaluationStub`; `DispatchEngine` now uses `Box<dyn Evaluator>` with `with_evaluator()` constructor. (2) `ProviderAdapter` bridges sync `ModelProvider` to async `Provider` via `Arc`+`spawn_blocking`; tier/token metadata flows through `ProviderRequest.metadata`. (3) `trait GraphOperations` defines `validate`/`topological_order`/`ready_nodes`; implemented for `DAGManager` and for `DependencyResolver` via `ResolvableGraph` wrapper. 9 new Rust tests (1305 total). `cargo fmt`, `cargo clippy`, handoff guard all pass. Phase 2 (Zero-Conflict Activation) is next.
- **2026-06-06**: Dormant Adaptation Phase 2 — Zero-Conflict Activation. (1) `DAGManager` activated in `ReadOnlyPlanner`: builds DAGManager from TaskDecomposer graph output, validates DAG structure, exposes topological order, supported mutations, and DAGMutationLimits metadata in plan output. (2) `context_pack` budget validation activated in `RuleBasedTaskAnalyzer`: `context_budget()` derives ContextBudget from TaskAnalysis, `context_pack_validation()` runs budget compliance and layer validation checks. (3) `WorkQueue` + `ResultAggregator` + `FeedbackIntegrator` with `AutoDowngradePolicy`/`AutoUpgradePolicy` activated in `WorkflowScheduler` tick: `SchedulerModules` struct holds all dormant module instances; WorkQueue tracks in-memory graph state alongside SQL persistence; FeedbackIntegrator records node outcomes and checks adaptation recommendations via auto_policies; ResultAggregator produces structured workflow result metadata on terminal runs. 13 new Rust tests (1318 total). `cargo fmt`, `cargo clippy`, handoff guard all pass. Phase 3 (Adapted Activation) is next.
- **2026-06-06**: Dormant Adaptation Phase 3 — Adapted Activation. (1) `QualityGateEvaluator` in `engine/src/quality/evaluator_bridge.rs` implements `Evaluator` trait, routing execution results through `FinalGateRunner` → `ArtifactGate` → `ScoringEngine` → `QualityGateManager` gate chain; opt-in via `DispatchEngine::with_evaluator()`. (2) `AdvisorBroker` from `harness/advisor` integrated into `DispatchEngine` via `with_advisor()`; advisory metadata enriches `execution_policy.advisory` with recommendation/confidence/reasoning/alternatives. (3) `ConflictResolver` + `HumanApprovalGate` activated in `WorkflowScheduler` tick via `SchedulerModules`: conflict detection (dependency/output/resource/budget) and resolution before aggregation; approval gate checks for failed/high-cost nodes. (4) `WorkflowEngine` activated in `WorkflowScheduler` as DAG-aware concurrency accelerator for multi-node runs with edges. `SchedulerModules` now holds 7 dormant module instances. 14 new Rust tests (1332 total). `cargo fmt`, `cargo clippy`, handoff guard all pass. Phase 4 (Dead Code Cleanup) is next.
- **2026-06-06**: Dormant Adaptation Phase 4 — Dead Code Cleanup. Removed ~9,400 lines across 44 files: `app_layer/` (16 files, fully duplicated by http_server + local_product_store), `dispatch/manual/` (7 files, superseded by CLI executor + scheduler), `harness/{sandbox,supervisor,batch_runner,sampling,model_eval}` (5 files, unused), `storage/{durable_store,health_checker,storage_migrator}` (3 files, superseded by LocalProductStore), `sdk.rs` (depended on removed DurableStore/HealthChecker), and 4 orphaned test files. `event_source/`, `errors.rs`, `event_schema.rs` tagged as reference-only. `test_backup_manager.rs` migrated to plain `rusqlite::Connection`. 1099 Rust tests pass, 0 failures. `cargo fmt`, `cargo clippy`, handoff guard all pass. All 4 dormant adaptation phases complete.
...
- Dormant Adaptation Phase 1 (Interface Unification): COMPLETE — trait Evaluator, ProviderAdapter, trait GraphOperations; 1305 tests
- Dormant Adaptation Phase 2 (Zero-Conflict Activation): COMPLETE — DAGManager in planner, context_pack in task_analyzer, WorkQueue+ResultAggregator+FeedbackIntegrator with AutoDowngrade/AutoUpgradePolicy in scheduler; 1318 tests
- Dormant Adaptation Phase 3 (Adapted Activation): COMPLETE — QualityGateEvaluator, AdvisorBroker in dispatch, ConflictResolver+HumanApprovalGate+WorkflowEngine in scheduler; 1332 tests
- Dormant Adaptation Phase 4 (Dead Code Cleanup): COMPLETE — removed ~9,400 lines (app_layer/, dispatch/manual/, harness/{sandbox,supervisor,batch_runner,sampling,model_eval}, storage/{durable_store,health_checker,storage_migrator}, sdk.rs, 4 test files); event_source/+errors+event_schema tagged reference-only; 1099 tests
...
- **Current count**: 1099 Rust tests pass, 0 failures (as of 2026-06-06)
