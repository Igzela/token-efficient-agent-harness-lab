...
- **2026-05-28**: Agent-Control-Plane Phase 7 Docker — Added local Docker deploy for Rust axum API and read-only Next.js dashboard. `docker compose build` passes. `docker compose up --build -d` smoke passed with alternate host ports (`ENGINE_PORT=18080`, `DASHBOARD_PORT=13000`) because local 8080 was already occupied; `/api/v1/health`, `/api/v1/dispatch`, and dashboard HTTP returned successfully. Remaining migration slice: final closeout and documentation reconciliation.
