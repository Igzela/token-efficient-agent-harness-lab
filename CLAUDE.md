...
- **2026-05-31**: Dashboard UX Polish — ARIA tab roles (role=tablist/tab/tabpanel) + keyboard arrow navigation, ConfirmDialog/KeyRevealModal focus traps (role=dialog, aria-modal, Escape key, Tab cycling), dispatch table rows keyboard-accessible (role=button, tabIndex, Enter/Space), form labels (htmlFor+id) on Team inputs, aria-label on SearchBar/refresh/theme-toggle, CSS spinner animation replacing plain text loading states, utility classes (.form-stack, .kv-row, .topbar-btn), structured dispatch detail key-value rendering replacing raw JSON dumps. 10 files changed, 0 new Rust tests (1161 total). TypeScript strict + readonly lint + Next.js build + static export all pass. Branch `feat/dashboard-ux-polish`.
...
- **Current count**: 1163 Rust tests pass, 0 failures (as of 2026-06-04)
