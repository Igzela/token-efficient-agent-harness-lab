...
fn append_memory_openapi_paths(doc: &mut Value) {
    ...
    paths.insert(
        ...
        json!({"post":{"summary":"Authorize one bounded provider-embedding retry","description":"Requires dispatch:execute, exact run/scope/target/attempt binding, and explicit confirmation. Known failures may retry directly. Unknown outcomes require immutable source/hash evidence that the provider had no effect. Authorization and audit commit atomically; attempts are capped at four.","parameters":[path_parameter("memory_id")],"requestBody":json_request_body(&["run_id","scope","target_version","expected_attempt_count","action","confirm_resolution"],json!({"run_id":{"type":"string"},"scope":{"type":"object","required":["tenant_id","workspace_id"]},"target_version":{"type":"integer","minimum":1},"expected_attempt_count":{"type":"integer","minimum":1,"maximum":3},"action":{"type":"string","enum":["retry_failed","confirm_unknown_no_effect_and_retry"]},"evidence_source_id":{"type":["string","null"]},"evidence_sha256":{"type":["string","null"],"pattern":"^[0-9a-f]{64}$"},"confirm_resolution":{"type":"boolean","const":true}})),"responses":{"200":{"description":"Hash-bound retry authorization"},"400":{"description":"Evidence, state, attempt, or confirmation invalid"},"403":{"description":"Run or operation scope mismatch"}}}}),
    );
    paths.insert(
