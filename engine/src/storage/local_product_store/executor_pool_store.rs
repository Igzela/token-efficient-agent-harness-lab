        ...
        match &self.db {
            DatabaseConnection::Sqlite(_) => self.with_conn(|conn| {
                ...
                for row in rows {
                    ...
                    entries.push(parse_executor_pool_entry(
                        &executor_type,
                        &caps_json,
                        &status_json,
                        &cost_json,
...
fn parse_executor_pool_entry(
    executor_type: &str,
    caps_json: &str,
    status_json: &str,
    cost_json: &str,
    metrics_json: &str,
) -> Result<ExecutorPoolEntry, String> {
    let caps_val: serde_json::Value = serde_json::from_str(caps_json).map_err(|e| e.to_string())?;
    let status_val: serde_json::Value =
        serde_json::from_str(status_json).map_err(|e| e.to_string())?;
    let cost_val: serde_json::Value = serde_json::from_str(cost_json).map_err(|e| e.to_string())?;
    ...
    let status = ExecutorStatus {
        ...
        cooldown_until: status_val["cooldown_until"].as_str().map(String::from),
    ...
    let metrics = ExecutorMetrics {
        ...
        last_executed_at: metrics_val["last_executed_at"].as_str().map(String::from),
    };

    Ok(ExecutorPoolEntry {
