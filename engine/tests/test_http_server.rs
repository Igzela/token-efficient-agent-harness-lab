...
fn make_generated_fixture() -> GeneratedAppFixture {
    let dir = tempdir().unwrap();
...
async fn generated_high_cost_candidate_id(app: &axum::Router, key: &str) -> String {
    let resp = app
        .clone()
        .oneshot(
            Request::builder()
...
struct AdaptivePolicyEnvCleanup;
